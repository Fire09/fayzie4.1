                ...
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then
                        if exports['fa-inventory']:hasEnoughOfItem('casinomember', 1) then
                            exports["fa-interface"]:showInteraction("Press [E] to play blackjack", 'info')
                        else
                            exports["fa-interface"]:showInteraction("Press [E] to play blackjack", 'error')
                        end
                    else 
                        exports["fa-interface"]:showInteraction("This seat is taken")
                        Citizen.Wait(3000)
...
AddEventHandler("Blackjack:successBlackjackBet",function()
    ...
    TriggerEvent("fa-blackjack:close")
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar then
            if timeLeft == 15 then 
                exports['fa-interface']:showInteraction('Time: '..timeLeft)
                Citizen.Wait(2500)
                exports['fa-interface']:hideInteraction()
...
AddEventHandler("betmenu", function()
    exports['fa-interface']:openApplication('textbox', {
        callbackUrl = 'fa-blackjack:betamount',
        key = 1,
        items = {
          {
            icon = "money-bill",
...
RegisterInterfaceCallback('fa-blackjack:betamount', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local chips = RPC.execute('fa-casino:getChips')
	...
	local menuCasino = {
		{
            ...
            key = "BJ.Hit",
			action = 'fa-blackjack:hit',
        },
		{
            title = "Stand",
            description = "Pussio Fam !",
            key = "BJ.Stand",
			action = 'fa-blackjack:stand',
        },
    }
    exports["fa-interface"]:showContextMenu(menuCasino)
end)

RegisterInterfaceCallback('fa-blackjack:stand', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('standcasino')
end)

RegisterInterfaceCallback('fa-blackjack:hit', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent("fa-blackjack:close", function()
    local pValues = {
        "line 1",
        "line 2",
    }
