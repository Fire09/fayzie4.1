        ...
        if DoesEntityExist(self.tableObject) and DoesEntityExist(self.ped) then
            ...
            if DoesEntityExist(self.tableObject) and DoesEntityExist(self.ped) then
                ...
                if DoesEntityExist(self.tableObject) and DoesEntityExist(self.ped) then
                    ...
                    TriggerEvent('DoLongHudText', '[Space] To Change Bet',1)
...
RegisterNetEvent('betmenu1')
AddEventHandler('betmenu1', function()
    local tmpInput = exports["fa-applications"]:KeyboardInput({
        header = "Bet Amount",
        rows = {
            {
...
function getBetObjectType(betAmount)
    ...
    elseif betAmount >= 10000 then -- this should never happen, but yeah.
        return GetHashKey('vw_prop_plaq_10kdollar_x1')
    end
