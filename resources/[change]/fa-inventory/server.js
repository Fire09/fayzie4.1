...
onNet("server-remove-item-kv", async (cid, itemId, amount, metaKey, metaValue) => {
    let src = source
    let playerInventory = 'ply-' + cid
    let query = `DELETE FROM inventory WHERE name = '${playerInventory}' and item_id = '${itemId}' AND (information LIKE '${metaKey}' OR information LIKE '${metaValue}') LIMIT ${amount}`;
...
onNet("server-inventory-remove-any", async (itemId, amount) => {
    let src = source
    let user = exports["fa-base"].getModule("Player").GetUser(src)
...
onNet("inventory:degItem", async (LastUsedItem, percent, LastUsedItemId, cid) => {
    ...
    exports["oxmysql"].execute(`UPDATE inventory SET creationDate = creationDate - ${amount} WHERE id = ${LastUsedItem} AND item_id = '${LastUsedItemId}' AND name = '${playerInventory}'`, {}, function() {});
});
