...
function DeleteOld() {
    ...
    for (let i = savelist.lengh - 1; i > 0; i--) {
        ...
        if (itemList[ItemID].fullyDegrades) {
            exports["oxmysql"].execute(`DELETE FROM inventory WHERE item_id = '${ItemID}' AND ${DeleteTime} > creationDate`);
        } else {
...
function CleanDroppedInventories() {
    exports["oxmysql"].execute(`DELETE FROM inventory WHERE dropped = '1'`)
    exports["oxmysql"].execute(`DELETE FROM inventory WHERE name LIKE '%_trash'`)
    DeleteOld()
}
