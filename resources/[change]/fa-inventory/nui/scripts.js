...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://fa-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://fa-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://fa-inventory/move', JSON.stringify(arr));
}
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://fa-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://fa-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://fa-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://fa-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://fa-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://fa-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://fa-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://fa-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://fa-inventory/ServerCloseInventory',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://fa-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://fa-inventory/invuse', JSON.stringify(arr));
...
function setPriceForItem(item,targetName, itemData) {
    if(priceEdit == ''){
        ...
        if(price >= 1){
            ...
            $.post('https://fa-inventory/invSetPrice', JSON.stringify(information));
