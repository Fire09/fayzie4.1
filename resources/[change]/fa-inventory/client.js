...
on('fa-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://fa-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('__cfx_nui:RepairSlot', (data, cb) => {
    ...
    emitNet('fa-inventory:repairSlot', slot);
    cb({});
});
...
let isCuffed = false;
RegisterNetEvent('fa-police:cuffs:state');
on('fa-police:cuffs:state', (pIsHandcuffed, pIsHandcuffedAndWalking) => {
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['fa-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
    ...
    // const apartmentFloor = exports["fa-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['fa-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
        ...
        if (!IsThisModelABicycle(vehicleModel) && !IsThisModelABike(vehicleModel)) {
            ...
            const vehId = exports['fa-vehicles'].GetVehicleIdentifier(vehicleFound)
            if (!vehId) {
        ...
        let serverCode = exports["fa-config"].GetServerCode();
...
function openTrunk(pEntity) {
    ...
    const vehId = exports['fa-vehicles'].GetVehicleIdentifier(pEntity)
    if (!vehId) {
        CloseGui();
...
on('fa-inventory:openDumpster', (pParams, pEntity, pContext) => {
    openDumpster(pEntity);
});

on('fa-inventory:openTrunk', (pParams, pEntity, pContext) => {
    openTrunk(pEntity);
});
...
RegisterNetEvent('fa-items:SetAmmo');
on('fa-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('fa-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("fa-inventory:craftProgression", data);
    emitNet("fa-inventory:craftProgression", data);
});

RegisterNuiCallbackType('weaponCrafted');
on('__cfx_nui:weaponCrafted', (data, cb) => {
    cb("ok");
    emitNet('fa-inventory:weaponCrafted', data.item);
});
...
async function CloseGui(pIsItemUsed = false) {
    ...
    //Remove blur
    // const forcedBlurred = exports['fa-miscscripts'].IsCurrentlyBlurred()
    // if (!forcedBlurred) {
...
//     if(exports["fa-housing"].isInsideProperty() && exports["fa-housing"].hasPermissionInProperty() == true && exports["fa-housing"].isPropertyShop())
//         return true
//     else
//         return false 
// }
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("fa-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("fa-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('fa-inventory:server:weightChange', cid, weight);
...
const doTranslations = async () => {
    // let isReady = exports['fa-i18n'].IsReady();
 {
        await Delay(1000);
        // isReady = exports['fa-i18n'].IsReady();
    }
    for (const key of Object.keys(itemList)) {
        ...
        //     // exports['fa-i18n'].GetStringSwap(itemList[key].__og_displayname || itemList[key].displayname);
        // itemList[key].information = 
        //     // exports['fa-i18n'].GetStringSwap(itemList[key].__og_information || itemList[key].information);
        // if (itemList[key].foodCategory) {
        //     itemList[key].foodCategory = (itemList[key].__og_food_cats || itemList[key].foodCategory)
        //         .map(c => exports['fa-i18n'].GetStringSwap(c));
...
on('fa-i18n:languageChanged', doTranslations);
...
let tradingIn = false;
on('fa-inventory:sionisReceipt', async ([pClass]) => {
    if (tradingIn) {
        return;
    }
    tradingIn = true;
    const [hasCraftAccess] = await RPC.execute('fa-business:hasPermission', 'sionis', 'craft_access');
    if (!hasCraftAccess) {
        tradingIn = false;
        return;
    }
    emit('doAnim', 'cokecut');
    const finished = await new Promise((r) => exports['fa-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
    ClearPedTasks(PlayerPedId());
    if (finished !== 100) {
        return;
    }
    ...
    matsToTrade.forEach((mat, k) => {
        ...
        const qty = Math.min(exports['fa-inventory'].getQuantity(mat, true), 50000);
    ...
    TriggerServerEvent('fa-inventory:sionisTaxedMats', taxedMats);
    canOpen = true;
    tradingIn = false;
});

on('fa-inventory:sionisReceiptTradeIn', async () => {
    const items = exports['fa-inventory'].getItemsOfType('refinedmaterialreceipt', 1000, true);
    if (items.length === 0) {
        return;
    }
    const [hasCraftAccess] = await RPC.execute('fa-business:hasPermission', 'sionis', 'craft_access');
...
on('fa-inventory:sionisCollectTaxes', () => {
    const ids = exports['fa-config'].GetMiscConfig('sionis.pickup.ids');
    const cid = exports['isPed'].isPed('cid');
    if (ids.indexOf(cid) === -1) {
        return;
    }
    TriggerServerEvent('fa-inventory:sionisCollectTaxes')
});

on('fa-inventory:openGoldRush', async () => {
    const [hasCraftAccess] = await RPC.execute('fa-business:hasPermission', 'gold_rush', 'craft_access');
    if (!hasCraftAccess) {
        return;
    }
...
on('fa-inventory:itemUsed', async (pItem, pInfo, pInv, pSlot) => {
    ...
    if (!meta.id) {
        ...
        meta = await RPC.execute('fa-inventory:getItemInformation', pInv, pSlot);
    }
    ...
    const prompt = await exports['fa-ui'].OpenInputMenu(
        [
            {
                label: 'Action',
                name: 'action',
    ...
    if (prompt.action === 'add') {
        for (const key of validKeys) {
            const qty = Math.min(exports['fa-inventory'].getQuantity(key, true), 50000);
            if (!qty) {
                continue;
            }
            if (!meta[key]) {
        ...
        TriggerServerEvent('fa-inventory:matBagUpdate', meta.id, meta);
        return;
    }
...
tradingIn = false;
on('fa-cia:depositMaterials', async () => {
    if (tradingIn) return;
    tradingIn = true;
    emit('doAnim', 'cokecut');
    const finished = await new Promise((r) => exports['fa-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
    ...
    const qty = Math.min(exports['fa-inventory'].getQuantity('cia_weapon_material', true), 50000);
    if (qty < 1) return;
    emit('inventory:removeItem', 'cia_weapon_material', qty);
    TriggerServerEvent('fa-inventory:depositCiaMaterials', qty);
});

on('fa-cia:checkMaterials', async () => {
    if (tradingIn) return;
    TriggerServerEvent('fa-inventory:checkCiaMaterials');
});

on('fa-cia:getGear', async () => {
    let response = 1;
    while (response) {
        await new Promise(r => setTimeout(r, 250));
        response = await exports['fa-ui'].OpenContextMenu([
            {
                key: { item: '-820634585' },
                title: 'Taser',
            },
        ...
        if (!response) break;
        TriggerServerEvent('fa-inventory:ciaGetGear', response.item);
    }
});

// AddEventHandler("fa-cia:getGear", function()
  
// end)

// AddEventHandler("fa-cia:checkMaterials", function()
