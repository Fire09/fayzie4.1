...
local function saveVehicle()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        TriggerEvent("fa-jobs:bennys:order", veh, changedStuff)
        return
    end
    local vehId = exports["fa-vehicles"]:GetVehicleIdentifier(veh)
...
function AttemptPurchase(purchaseType, upgradeLevel)
    ...
    if bennys.access and (bennys.access == "police" or bennys.access == "ems") and purchaseType == "repair" then
        return RPC.execute("fa-bennys:jobRepair")
    end
    ...
    if (bennys.access == "police" or bennys.access == "ems") then
        local free = exports['fa-config']:GetMiscConfig('bennys.pd.free')

        pd = free
    end

    TriggerServerEvent("fa-bennys:attemptPurchase", cheap, purchaseType, upgradeLevel)
...
function RepairVehicle()
    Citizen.CreateThread(function ()
        ...
        if (missingEngineHealth > 50) then
            local finished = exports["fa-taskbar"]:taskBar(5000 + (missingEngineHealth / 50), "Repairing engine...", true)
        ...
        if missingBodyHealth > 50 then
            local finished = exports["fa-taskbar"]:taskBar(5000 + (missingBodyHealth / 50), "Repairing body...", true)
...
function PreviewMod(categoryID, modID)
    ...
    if categoryID == 39 or categoryID == 4 then
        if GetResourceState("fa-vehiclesync-state") == "started" then
            exports['fa-vehiclesync-state']:SetEngineSound(plyVeh, true)
        elseif GetResourceState("fa-vehiclesync") == "started" then
            exports['fa-vehiclesync']:SetEngineSound(plyVeh, true)
        end
    end
end
...
function ExitBennys()
    ...
    if isAtEmployedBennys and not isAtPreviewBennys then
        ...
        if GetResourceState("fa-vehiclesync-state") == "started" then
            exports['fa-vehiclesync-state']:SetEngineSound(plyVeh)
        elseif GetResourceState("fa-vehiclesync") == "started" then
            exports['fa-vehiclesync']:SetEngineSound(plyVeh)
        end
    ...
    TriggerServerEvent("fa-bennys:removeFromInUse", currentBennys)
    if devmode then
      TriggerEvent("vehicle:leftBennys", nil, nil)
    elseif not isAtPreviewBennys then
      exports["fa-ui"]:showInteraction("Bennys")
...
local function finishEnterLocation()
    ...
    isPlyInBennys = true
    TriggerServerEvent("fa-bennys:addToInUse", currentBennys)
    disableControls()
    TriggerEvent("inmenu", isPlyInBennys)
end
...
function enterLocation(pBennys, needsRepair)
    ...
    if isAtEmployedBennys then
        ...
        VehicleClass = exports["fa-vehicles"]:GetVehicleRatingClass(plyVeh)
    end
    ...
    isVinScratched = exports['fa-vehicles']:IsVinScratched(plyVeh)
...
--#[Event Handlers]#--
RegisterNetEvent("fa-bennys:purchaseSuccessful")
AddEventHandler("fa-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("fa-bennys:purchaseFailed")
AddEventHandler("fa-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    local isBennysInUse = RPC.execute("fa-bennys:checkIfUsed", currentBennys)
    ...
    if bennysAccess then
        ...
        jobAccess = exports["fa-business"]:IsEmployedAt(bennysAccess)

        if (isAtPreviewBennys and previewPermission[currentBennys] and isTestDrive) then
            jobAccess = exports["fa-business"]:HasPermission(bennysAccess, previewPermission[currentBennys])
        end
...
Citizen.CreateThread(function()
    Wait(5000)
    local catalog = RPC.execute('fa-jobs:bennys:getPartsCatalog')

    local modifiers = RPC.execute('fa-jobs:bennys:getPriceModifiers')

    -- local taxLevel = RPC.execute("fa-financials:getTax", "Services")
...
RegisterNetEvent('fa-admin:bennys', function()
    devmode = true
