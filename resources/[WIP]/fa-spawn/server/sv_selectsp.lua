...
AddEventHandler("character:loadspawns", function()
    ...
    local name = GetPlayerName(src)
    exports.oxmysql:execute("SELECT ho.*, cm.cid, cm.building_type, hp.level, hp.illness, hp.time FROM character_motel cm LEFT JOIN __housedata ho on ho.cid = cm.cid LEFT JOIN hospital_patients hp on hp.cid = cm.cid WHERE cm.cid = @cid ",{
        ["@cid"] = cid
        ...
        exports.oxmysql:execute('SELECT * FROM __housedata WHERE `cid` = @id', {
            ...
            if housingMotels[1] then
                ...
                spawnData["houses"] = exports["fa-housing"]:getCurrentOwned(src)
                spawnData["keys"] = exports["fa-housing"]:currentKeys(src)
