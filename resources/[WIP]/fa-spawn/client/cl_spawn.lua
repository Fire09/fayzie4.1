...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = exports["fa-config"]:GetMiscConfig("characters.lifers.disabled")
	local hardcoreDisabled = exports["fa-config"]:GetMiscConfig("characters.hardcore.disabled")
...
function Login.NUICallback(data)
	...
	if data.action == "newCharacter" then
		...
		if cData.type and cData.type == 'hardcore' then
			local canCreate = RPC.execute('fa-spawn:canCreateHardcoreCharacter')
