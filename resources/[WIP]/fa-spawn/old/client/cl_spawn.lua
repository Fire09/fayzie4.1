...
function Login.nativeStart(AlreadySpawned)
	...
	if not AlreadySpawned then
		tatCategory = exports["fa-clothing"]:GetTatCategs()
		tattooHashList = exports["fa-clothing"]:GetTatCategs()
		Login.custompeds = exports["fa-clothing"]:GetCustomSkins()
...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["fa-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		events:Trigger("fa-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		TriggerServerEvent('fa-spawn:createCharacter:log', cData.first_name, cData.last_name, cData.dob, pGender)
		
        if not chardata then return end

		events:Trigger("fa-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("fa-base:spawnInitialized")
