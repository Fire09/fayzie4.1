...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['fa-base']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("fa-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RegisterNetEvent("fa-housing:enterHouse")
AddEventHandler("fa-housing:enterHouse", function(pPropertyId)
    local src = source
    Housing.Players[src] = pPropertyId
end)

RegisterNetEvent("fa-housing:leftHouse")
AddEventHandler("fa-housing:leftHouse", function(pPropertyId)
    local src = source
...
RPC.register("fa-phone:getHouseKeys", function(src, pHouseId)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return {} end
...
RPC.register("fa-phone:giveKey", function(src, pHouseId, pPlayerId)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false, "ID não encontrado" end

    local user2 = exports['fa-base']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("fa-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
    TriggerClientEvent("fa-housing:refresh", pPlayerId)

    return true, "Chave recebida"
end)

RPC.register("fa-phone:removeKey", function(src, pHouseId, pPlayerId)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
...
RPC.register("fa-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local accountId = exports["fa-base"]:getChar(src, "bankid")
   -- local bank = exports["fa-financials"]:getBalance(accountId)
   -- if bank < priceTotal then
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    --end

    --local groupBank = exports["fa-groups"]:groupBank("real_estate")

    --local comment = "Pagamento de aluguel da propriedade " .. Housing.info[pHouseId]["street"]
    --local success, message = exports["fa-financials"]:transaction(accountId, groupBank, price, comment, cid, 5)
    --if not success then
      --  return false, message
    --end

    --exports["fa-financials"]:addTax("Propertys", tax.tax)
...
RPC.register("fa-phone:removeSharedKey", function(src, pHouseId)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false end
    ...
    if affectedRows and affectedRows > 0 then
        TriggerClientEvent("fa-housing:refresh", src)
        return true
    end
...
RegisterServerEvent('fa-housing:buyLog')
AddEventHandler('fa-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    ...
    PerformHttpRequest("https://discord.com/api/webhooks/1012082840038555690/uBrYz4sosUN9pwxpHI6h13f87WB8HQzvUcVFPhXTI6xJOsvsbltkRUGccok-4ICS51PG", function(err, text, headers) end, 'POST', json.encode({username = "faye | House Bought Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)