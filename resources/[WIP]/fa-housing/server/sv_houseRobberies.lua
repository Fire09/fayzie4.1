...
RegisterNetEvent("fa-base:sessionStarted", function()
    local src = source
    TriggerClientEvent("fa-housing:beingRobbed", src, Housing.housingBeingRobbed)
end)
...
RPC.register("housing:robbery:pickedLock", function(src, pHouseId)
    ...
    TriggerClientEvent("fa-housing:beingRobbed", -1, pHouseId)
...
RPC.register("property:alarmDisabled", function(src, pHouseId, pFail)
    ...
    for k, v in pairs(Housing.Players) do
        if v == pHouseId then
            ...
            if pFail then
                TriggerClientEvent("fa-housing:alarm", src, pHouseId)
            end
        end
    end
