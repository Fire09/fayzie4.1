...
RegisterNetEvent("fa-housing:sell")
AddEventHandler("fa-housing:sell", function(pCid, pPropertyId)
    local src = source
    ...
    print('here')
    TriggerClientEvent("fa-housing:buy", pCid, pPropertyId, seller)
end)
...
AddEventHandler("CheckFurniture", function(pData, pPropertyId)
    ...
    if housingEditing[pPropertyId] ~= nil then
        ...
        TriggerClientEvent("fa-editor:loadEditor", src, pData)
    end
end)
...
RPC.register("fa-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("fa-housing:loadHousingClient",src)
    TriggerClientEvent('fa-housing:logShit', src, pPropertyId, pTotal)
    return true, "Purchase Successful" 
end)

RPC.register("fa-housing:rent", function(src, pPropertyId, pTotal, pTax)
    local user = exports['fa-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    ...
    --local success, message = exports["fa-financials"]:transaction(accountId, groupBank, pTotal, comment, cid, 5)
    --if not success then
    --    return false, message
    --end

    --exports["fa-financials"]:addTax("Propertys", pTax)
...
RPC.register("updateCurrentSelected", function(src, pPropertyId, pInformation, pOrigin)
    ...
    --exports["fa-vehicles"]:setGarage(name, data, nil, false)
...
RPC.register("fa-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RPC.register("fa-housing:owned", function(src)
...
RegisterNetEvent("fa-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:fa-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)