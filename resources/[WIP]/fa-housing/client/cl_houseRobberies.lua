...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["fa-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["fa-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    if force and Housing.currentlyDisplayingPickup == true then
        Housing.currentlyDisplayingPickup = false
        exports["fa-textui"]:hideInteraction()
    end

    if Housing.currentlyDisplayingPickup == false and not force then
        Housing.currentlyDisplayingPickup = true
        if type(itemName) == "string" then
            exports["fa-textui"]:showInteraction("[E] Take Object")
        else
            exports["fa-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["fa-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["fa-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["fa-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("fa-heists:complete", math.random(30, 50))
...
RegisterNetEvent("fa-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("fa-housing:alarmFail")
AddEventHandler("fa-housing:alarmFail", function()
...
RegisterNetEvent("fa-housing:alarmEnter")
AddEventHandler("fa-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("fa-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("fa-housing:alarm")
AddEventHandler("fa-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["fa-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("fa-housing:beingRobbed", function(pPropertyId)
