...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords, objects = exports["fa-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["fa-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        TriggerServerEvent("fa-housing:enterHouse", propertyID)

        if not counterPart then
            TriggerEvent("fa-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
RegisterNetEvent("fa-housing:loadHousingClient")
AddEventHandler("fa-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("fa-housing:reset")
AddEventHandler("fa-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("fa-housing:refresh")
AddEventHandler("fa-housing:refresh", function()
    gatherPlayerInfo()
end)
