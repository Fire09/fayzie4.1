...
RegisterNetEvent("fa-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("fa-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("fa-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('fa-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("fa-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('fa-phone:editingHouse',id,street,price)
    else
        TriggerEvent('fa-phone:sentHouse',nil,id,street,price,category)
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["fa-build"]:getModule("func").currentBuildingVector()
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("fa-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["fa-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("fa-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("fa-housing:sellTo", function(pParams)
    local input = exports["fa-ui"]:showInput({
		{
            icon = "id-card",
            label = "PayPal ID",
            name = "cid",
	...
	if input["cid"] then
		...
		TriggerServerEvent("fa-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("fa-housing:buy")
AddEventHandler("fa-housing:buy", function(pPropertyId, pSeller)
...
RegisterCommand('rent', function()
    TriggerEvent('fa-housing:rent')
end)

RegisterNetEvent("fa-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("fa-housing:rent2", function(pParams)
    local complete, info = RPC.execute("fa-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("fa-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("fa-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("fa-housing:edit", function()
end)

RegisterNetEvent("fa-housing:editOptions", function(pParams)
...
AddEventHandler("housing:charLogout", function()
    ...
    if max.canHaveCharSelect then
        ...
        exports["fa-ui"]:sendAppEvent("hud", { display = false })
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["fa-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["fa-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["fa-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("fa-housing:blips")
AddEventHandler("fa-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("fa-housing:owned")
...
RegisterNetEvent('fa-housing:logShit')
AddEventHandler('fa-housing:logShit', function(hID, Price)
    TriggerServerEvent('fa-housing:buyLog', hID, Price)
end)