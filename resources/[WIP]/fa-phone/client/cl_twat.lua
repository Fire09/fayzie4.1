...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("fa-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('fa-phone:twatSendNotification', handle, data, data.time)
...
RegisterNUICallback("fa-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('fa-phone:insertTwat')
AddEventHandler('fa-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
