...
RPC.register('fa-phone:debt', function(source)
    local src = source
...
function buildInfo(house)
    ...
    for k,v in pairs(house) do
        ...
        local street = exports['fa-housing']:HouseStreet(v.hid)
...
RPC.register('fa-phone:debtPrice', function(source, id)
    local src = source
    local hId = id
    return getPrice(hId)
end)
...
RPC.register('fa-phone:car_payment', function(source, pl)
    local src = source
    local plate = pl
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    local status
...
RPC.register('fa-phone:h_payment', function(source,id,pay)
    local src = source
    local hId = id
    local payment = pay
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
...
RPC.register('fa-phone:p_payment', function(source,id,p,did)
    local src = source
    local pId = id
    local dId = did
    local payment = p
...
RPC.register('fa-phone:addBill', function(source,id,a,t,c)
    local src = source
    local cid = id
    local amount = a
    local type = t
