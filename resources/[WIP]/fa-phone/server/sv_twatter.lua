...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('fa-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('fa-phone:twatSendNotification')
AddEventHandler('fa-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('fa-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('fa-phone:openGetTweets')
AddEventHandler('fa-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('fa-phone:getTwatters', function(source)
...
RPC.register("fa-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["fa-base"]:getModule("Player"):GetUser(src)
