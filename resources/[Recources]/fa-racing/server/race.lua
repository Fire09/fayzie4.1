...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("fa-racing:joinedRace", src, Races[id])
    TriggerClientEvent("fa-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("fa-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("fa-racing:updatePositionInfo")
AddEventHandler("fa-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("fa-racing:updatePosition", src, position)
end)
...
RPC.register("fa-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("fa-racing:addedPendingRace", -1, options)
    TriggerClientEvent('fa-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("fa-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("fa-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("fa-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("fa-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("fa-racing:leftRace", src)
        TriggerClientEvent("fa-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("fa-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("fa-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("fa-racing:leftRace", src)
    TriggerClientEvent("fa-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("fa-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("fa-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("fa-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("fa-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("fa-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("fa-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("fa-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['fa-base']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("fa-racing:endRace", function(src)
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("fa-racing:removedActiveRace", -1, raceId)
end)

RPC.register("fa-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("fa-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("fa-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("fa-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("fa-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("fa-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("fa-racing:dnfRace", v.serverId)
            end
        end
