...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('fa-paleto:getGetDoorStateSV')
...
RegisterNetEvent("fa-paleto:getDoorCheckCL")
AddEventHandler("fa-paleto:getDoorCheckCL", function(result)
...
RegisterNetEvent("fa-paleto:resetDoorState")
AddEventHandler("fa-paleto:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("fa-paleto:lootup_c")
AddEventHandler("fa-paleto:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("fa-paleto:outcome")
AddEventHandler("fa-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("fa-paleto:startheist", Paleto.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("fa-paleto:startLoot_c")
AddEventHandler("fa-paleto:startLoot_c", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("fa-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("fa-paleto:stopHeist_c")
AddEventHandler("fa-paleto:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("fa-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("fa-paleto:ClosePaletoDoor")
AddEventHandler("fa-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        ...
        Citizen.Wait(10000)
    until count == 900
    TriggerServerEvent('fa-paleto:getGetDoorStateSVSV', false)
end)


-- RegisterNetEvent("fa-paleto:toggleVault")
...
--             TriggerServerEvent("fa-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
RegisterNetEvent("fa-paleto:OpenPaletoDoor")
AddEventHandler("fa-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('fa-paleto:getGetDoorStateSVSV', true)
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418, 6472.1274414062, 31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
...
AddEventHandler("fa-paleto:reset", function(name, data)
    ...
    TriggerServerEvent("fa-paleto:closeDoor")
    -- TriggerServerEvent("fa-paleto:toggleVault", name, true)
    TriggerEvent("fa-paleto:cleanUp", data, name)
end)


AddEventHandler("fa-paleto:startheist", function(data, name)
    disableinput = true
    currentname = name
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent('fa-paleto:openDoor')
    TriggerServerEvent("fa-paleto:setCooldown", name)
    --TriggerServerEvent("fa-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

-- AddEventHandler("fa-paleto:cleanUp", function(data, name)
--     Citizen.Wait(10000)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("fa-paleto:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("fa-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('fa-paleto:darkmail')
AddEventHandler('fa-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('fa-paleto:UseBlueLapTop')
AddEventHandler('fa-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("fa-paleto:getBanks")
	TriggerEvent("fa-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["fa-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            Citizen.SetTimeout(timeout, function()
                                TriggerServerEvent("fa-paleto:startcheck", k)
                            end)
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('fa-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('fa-doors:change-lock-state', 517, false)
end)
