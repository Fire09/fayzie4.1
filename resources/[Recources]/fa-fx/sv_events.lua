...
RegisterServerEvent("fa-fx:setSteamId")
AddEventHandler("fa-fx:setSteamId", function()
    local src = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("fa-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("fa-fx:smoke:grenade")
AddEventHandler("fa-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("fa-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
