...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("fa-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
function RepairVehicle()
    ...
    ExitBennys()
    local finished = exports["fa-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    if currentBennys:find('^devbennys') then
        ...
        exports["fa-interface"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
AddEventHandler("bennys:enter", function()
    ...
    if currentBennys == "bennystuner" then
        if exports['fa-business']:IsEmployedAt('tuner_shop') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "bennyshayes" then
        if exports['fa-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "bennysvinewood" then
        if exports['fa-business']:IsEmployedAt('') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['fa-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['fa-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
...
RegisterNetEvent("Evolve:admin:currentDevmode")
AddEventHandler("Evolve:admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)
...
RegisterNetEvent("fa-bennys:repairVeh")
AddEventHandler('fa-bennys:repairVeh', function()
    ...
    if car ~= 0 then
        ...
        local finished = exports["fa-taskbar"]:taskBar(4000,"Repairing Engine",false,false,playerVeh)
        Citizen.Wait(1000)
        local finished = exports["fa-taskbar"]:taskBar(3000,"Repairing Body",false,false,playerVeh)
        if (finished == 100) then
...
RegisterNetEvent('fa-public-bennys')
AddEventHandler('fa-public-bennys', function()
    TriggerServerEvent('fa-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["fa-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
...
RegisterNetEvent('fa-vinewoord-bennys')
AddEventHandler('fa-vinewood-bennys', function()
    TriggerServerEvent('fa-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["fa-polyzone"]:AddBoxZone("bennysvinewood", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=116.9,
        minZ=25.72,
        maxZ=29.52
    })
