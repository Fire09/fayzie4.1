RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("fa-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("fa-musicplayer:stateSound")
AddEventHandler("fa-musicplayer:stateSound", function(state, data)
