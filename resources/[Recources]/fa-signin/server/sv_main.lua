...
RegisterServerEvent("fa-signin:duty")
AddEventHandler("fa-signin:duty", function(pJobType, pSrc)
    ...
    jobs:SetJob(user, pJobType, false, function()
        if pJobType == "police" then
            exports['fa-dispatch']:callsign_command()
...
RegisterServerEvent("fa-signoff:duty")
AddEventHandler("fa-signoff:duty", function(job)
	local src = source
    if job == 'police' then
