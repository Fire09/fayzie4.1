...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["fa-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["fa-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    if(data.toSelect){
        global.exports["fa-admin"].enterSelection();
    }
...
on("onResourceStart", (resourceName) => {
    if(GetCurrentResourceName() != 'fa-admin') {
      return;
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    if(data.inside){
        ...
        emit('fa-binds:should-execute',false)
        ...
        emit('fa-binds:should-execute',true)
        if(blockingActions){
...
global.exports["fa-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
