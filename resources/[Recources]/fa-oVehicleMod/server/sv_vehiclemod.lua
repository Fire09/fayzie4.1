...
AddEventHandler("vehicleMod:getHarness", function(plate)
    ...
    exports.oxmysql:execute("SELECT harness FROM characters_cars WHERE license_plate = @plate", {['plate'] = plate}, function(result)
...
AddEventHandler("vehicleMod:applyHarness", function(plate, durability)
    ...
    exports.oxmysql:execute("UPDATE characters_cars SET harness = @durability WHERE license_plate = @plate",
    {
        ['plate'] = plate,
...
AddEventHandler("transfer:attempt:send", function(plate, target)
    ...
    exports.oxmysql:execute("SELECT * FROM characters_cars WHERE cid = ? AND license_plate = ?",{pChar, plate}, function(data)
        if data[1] then
            exports.oxmysql:execute("UPDATE characters_cars SET `cid` = @cid, `owner` = @owner WHERE license_plate = @license_plate", {
                ['cid'] = tChar, 
                ['owner'] = tSteam,
                ['license_plate'] = plate
            })
