...
local curJob = "none"
RegisterNetEvent("cool-jobmanager:playerBecameJob")
AddEventHandler("cool-jobmanager:playerBecameJob", function(job)
  curJob = job
end)
  ...
  ["resfooditem"] = {
    ...
    restaurantBaseBuff = {
      ["burger_shot"] = function(info)
        ...
        TriggerEvent("cool-buffs:applyBuff", "burger_shot", { { buff = "stressblock", percent = 1.0, timeOverride = 180 * 60000 } })
        TriggerEvent("hadsugar")
        return processInfo(info)
      end,
      ["rooster"] = function(info)
        TriggerEvent("cool-buffs:applyBuff", "rooster", { { buff = "jobluck", percent = 1.0 } })
        return processInfo(info)
      end,
      ["uwu_cafe"] = function(info)
        TriggerEvent("cool-buffs:applyBuff", "uwu_cafe", { { buff = "strength", percent = 0.5, timeOverride = 60000 * 60 } })
        return processInfo(info)
      end,
      ["maldinis"] = function(info)
        TriggerEvent("cool-buffs:applyBuff", "maldinis", { { buff = "fullness", percent = 1.0 } })
        return processInfo(info)
      end,
    },
  },
...
AddEventHandler("cool-buffs:itemUsedSuccess", function(pSuccess)
  itemUseSuccess = pSuccess
end)
AddEventHandler("cool-inventory:itemUsed", function(pItem, pInfo)
  local config = buffConfig[pItem]
  if not config then return end
  itemUseSuccess = nil
  TriggerEvent("cool-inventory:attachPropPlayAnim", config.attachItem)
  ...
  TriggerEvent("cool-buffs:applyBuff", pItem, buffs)
