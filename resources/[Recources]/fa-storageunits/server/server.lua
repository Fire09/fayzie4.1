RPC.register("fa-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("fa-storageunits:purchase", function(args)
    local pSrc = source
...
RPC.register("fa-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(pSrc)
        ...
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "fa-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "fa-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('fa-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "fa-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("fa-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
...
RPC.register("fa-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("fa-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(pSrc)
    exports["oxmysql"]:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1].given_access ~= nil then
            ...
            for i=1, #current_keys do  
                exports["oxmysql"]:execute("SELECT `first_name`, `last_name`, `id` FROM characters WHERE id = @id", {['id'] = current_keys[i]}, function(result)
                    if result[1] ~= nil then
                        for i=1, #result do
                            TriggerClientEvent('fa-context:sendMenu', pSrc, {
                                {
                                    id = result[i].id,
                                    header = "Name: " ..result[i].first_name .. " " .. result[i].last_name,
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "fa-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
RPC.register("fa-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
