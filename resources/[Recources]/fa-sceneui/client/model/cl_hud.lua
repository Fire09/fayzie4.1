...
--
local cmdsSet = {}
RegisterNUICallback("fa-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["fa-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["fa-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
...
AddEventHandler("fa-ui:watch", function()
...
AddEventHandler("Faye:admin:currentDevmode", function(devmode)
    setGameValue("modeDev", devmode)
end)

RegisterNetEvent("Faye:admin:currentDebug")
AddEventHandler("Faye:admin:currentDebug", function(debugToggle)
...
AddEventHandler('fa-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('fa-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('fa-weapons:client:setWeaponFireRate')
AddEventHandler('fa-weapons:client:setWeaponFireRate', function (pAmount)
...
RegisterNetEvent('fa-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["fa-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["fa-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["fa-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["fa-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["fa-ui"]:sendAppEvent("hud", { hasParachute = true })
end)
