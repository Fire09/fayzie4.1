...
$(function() {
    sleep(5000,function(){
        $.post('https://fa-sceneui/fa-sceneui:getKVPValue', JSON.stringify({
            key : "fa-preferences",
        }),function(result){
            ...
            $.post('https://fa-sceneui/fa-sceneui:hudSetPreferences', JSON.stringify(settings),function(result){
...
document.querySelector('#setting_hud_save').addEventListener('click', function(){
    ...
    $.post('https://fa-sceneui/fa-sceneui:setKVPValue', JSON.stringify({
        key : "fa-preferences",
        value:settings
    }),function(res){
        ...
        if(data_captcha.solution === answer){
            ...
            if( (mode === 'vault' && streakcaptcha === roundTotal_captcha) ){
                ...
                sleep(5000, function(){ 
                    ...
                    $.post('https://fa-sceneui/'+minigame_captcha_endpoint, JSON.stringify({
                        result : true
                    }));
                });
            }else{
...
function correct_skillbar(){
    ...
    $.post('https://fa-sceneui/fa-sceneui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
function reset(checked){
    ...
    if (checked) {
        ...
        sleep(4000, function(){
            ...
            $.post('https://fa-sceneui/'+minigame_memory_endpoint, JSON.stringify({
                result : true
            }));
        })
    }else{
...
function checkanswernumbers(){
    ...
    if(game_started_numbers && response === answer_numbers.join('')){
        ...
        sleep(3000, function(){
            ...
            $.post('https://fa-sceneui/'+gameFinishedEndpointNumbers, JSON.stringify({
                success : true
            }));
        });
    }else{
...
document.querySelector('#textbox_submit').addEventListener('click', function(){
    ...
    $.post('https://fa-sceneui/fa-sceneui:closeApp', JSON.stringify({}),function(res){
        $.post('https://fa-sceneui/'+textCallback, JSON.stringify({
            key : textKey,
            values : textData,
        }),function(cb){
            textCallback = null;
            ...
            $.post('https://fa-sceneui/fa-sceneui:applicationClosed', JSON.stringify({
                name : "textbox",
                fromEscape : false
            }));
        });
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (action) {
                    if (children) {
                        ...
                        $.post('https://fa-sceneui/'+action, JSON.stringify({
                            key:key
                        }));
                    }else{
                        contextActive = false;
                ...
                $.post('https://fa-sceneui/'+subaction, JSON.stringify({
                    key:subkey
                }),function (res) { 
...
document.querySelector('.vol-up-wrapper').addEventListener('click', function(){
    $.post('https://fa-sceneui/fa-sceneui:radioVolumeUp', JSON.stringify({}));
});

document.querySelector('.vol-down-wrapper').addEventListener('click', function(){
    $.post('https://fa-sceneui/fa-sceneui:radioVolumeDown', JSON.stringify({}));
});

document.querySelector('.on-off-wrapper').addEventListener('click', function(){
    radiostate = !radiostate
    if (radiostate == true) {
        $.post('https://fa-sceneui/fa-sceneui:toggleRadioOn', JSON.stringify({}));
    }else if (radiostate == false) {
        $.post('https://fa-sceneui/fa-sceneui:toggleRadioOff', JSON.stringify({}));
    }
});
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://fa-sceneui/fa-sceneui:targetSelectOption', JSON.stringify({
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        if (showroomuiactive) {
            ...
            $.post('https://fa-sceneui/fa-sceneui:closeApp', JSON.stringify({}),function (cb) { 
                $.post('https://fa-sceneui/fa-sceneui:applicationClosed', JSON.stringify({
                    name : "showroom",
                    fromEscape : true
                }));
