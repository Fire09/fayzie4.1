...
RegisterNetEvent('fa-doors:initial-lock-state')
AddEventHandler('fa-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
AddEventHandler('fa-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local devmode = false
RegisterNetEvent("faye:admin:currentDevmode")
AddEventHandler("faye:admin:currentDevmode", function(pDevmode)
    devmode = pDevmode
end)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["fa-interface"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        exports["fa-interface"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                        exports["fa-interface"]:showInteraction((newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("fa-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["fa-interface"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("fa-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local entity = exports['fa-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent("fa-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["fa-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["fa-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    TriggerServerEvent("fa-doors:request-lock-state")
end)

RegisterCommand("addnewdoor", function()
    local pEntity = exports['fa-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    TriggerServerEvent("fa-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
end, false)

RegisterNetEvent("fa-doors:add")
AddEventHandler("fa-doors:add", function(pArgs, pEntity, pContext)
...
RegisterNetEvent("fa-doors:detCoord")
AddEventHandler("fa-doors:detCoord", function ()
  ...
  Citizen.CreateThread(function()
    ...
    local x, y, z = table.unpack(GetEntityCoords(bomba))
    print(json.encode(jesus))
    TriggerEvent("qb-doors:client:ThermitePtfx", jesus)
    Citizen.Wait(5000)
    ...
    ClearPedTasks(ped)  
    DeleteObject(bomba)
    TriggerServerEvent("fa-doors:change-lock-state", targetDoor, 0)
