...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
   -- dwRefreshChipCount = false
    dwCurrentChipCount = RPC.execute("fa-casino:getCurrentChipCount")
  end
...
--   exports["fa-ui"]:sendAppEvent("status-hud", {
--     show = true,
--     title = title,
--     values = values,
--   })
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['fa-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                            canPlayAtCurrentTable = true
                        elseif exports['fa-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                            background = "success"
                            canPlayAtCurrentTable = true
                        else
                            background = "error"
                        ...
                        -- exports["fa-ui"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["fa-ui"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['fa-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['fa-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
...
RegisterUICallback("fa-ui:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("fa-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("fa-casino:winChips", currentBetAmount / 2)
...
AddEventHandler("fa-ui:application-closed", function(name)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "fa-ui:blackjackHitStand",
...
RegisterUICallback("fa-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("fa-ui:application-closed", function(name, data)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("fa-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("fa-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
