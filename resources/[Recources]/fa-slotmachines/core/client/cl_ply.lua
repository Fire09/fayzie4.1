...
local function drawStatusHUD(state, pValues)
  ...
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("fa-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["fa-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "fa-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "fa-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["fa-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            exports["fa-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
AddEventHandler("fa-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("fa-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("fa-casino:casinoExitedEvent", function()
  ...
  isSlotMachinesEnabled = RPC.execute("fa-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("fa-textui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("fa-casino:useChips", currentBet)
    ...
    local user = exports["fa-fw"]:getModule("LocalPlayer")
...
RegisterInterfaceCallback("fa-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
