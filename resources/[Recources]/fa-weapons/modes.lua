...
Citizen.CreateThread(function()
	...
	-- Register Keybinds
	exports["fa-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("fa-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
	...
	if selectedRate.label == "Single Fire" then
		TriggerEvent('fa-weapons:client:setWeaponFireRate', 30)
	elseif selectedRate.label == "Burst Fire" then
		TriggerEvent('fa-weapons:client:setWeaponFireRate', 70)
	elseif selectedRate.label == "Full Auto" then
		TriggerEvent('fa-weapons:client:setWeaponFireRate', 100)
	end	
end
...
RegisterNetEvent("fa-weapons:client:weaponEquiped")
AddEventHandler("fa-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("fa-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("fa-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("fa-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
