...
AddEventHandler('stress:illnesslevel', function(newval)
 ...
 --   exports.oxmysql:execute("UPDATE hospital_patients SET level = @newval  WHERE cid = @id",{['newval'] = newval, ['id'] = char.id})
end)
...
AddEventHandler('stress:illnesslevel:new', function(injury,lenghtofinjury)
    ...
    local char = user:getCurrentCharacter()
    local newval = tonumber(newval)
   -- exports.oxmysql:execute("UPDATE hospital_patients SET level = @lenghtofinjury, illness = @injury, time = @time  WHERE cid = @id",{['time'] = 60, ['injury'] = injury, ['id'] = char.id, ['lenghtofinjury'] = lenghtofinjury})
end)
