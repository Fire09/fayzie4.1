...
AddEventHandler('fa-items:SetAmmo', function(sentammoTable)
...
function actionBarDown()
  ...
  exports["fa-ui"]:sendAppEvent("hud", {
    displayAllForce = true,
    displayAllForceVehicle = GetVehiclePedIsIn(PlayerPedId()) ~= 0,
	displayRadioChannel = true,
  })
  TriggerServerEvent("fa-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["fa-keybinds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["fa-keybinds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
	RegisterCommand('+playerWink', playerWink, false)
	RegisterCommand('-playerWink', function() end, false)

	exports["fa-keybinds"]:registerKeyMapping("", "Player", "Passive Mode", "+passiveMode", "-passiveMode")
...
AddEventHandler('fa-actionbar:hotreload', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("fa-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["fa-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["fa-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["fa-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("fa-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["fa-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	TriggerEvent("fa-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("fa-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("fa-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('fa-item:CheckClientAmmo')
AddEventHandler('fa-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["fa-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["fa-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
...
RegisterNetEvent("fa-weapons:hitPlayerWithCash", function(pTarget)
  TriggerServerEvent("fa-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
