...
RegisterNetEvent('fa-binds:keyEvent')
AddEventHandler('fa-binds:keyEvent', function(name,onDown)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    ...
    local isBuiltCoords = exports["fa-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            ...
            TriggerEvent("fa-spawn:characterSpawned", cid)
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["fa-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["fa-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["fa-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
...
function Apart.OpenStash()
    if not exports['fa-taskbar']:pInInv() then
        local cid = exports["isPed"]:isPed("cid")
...
function garageToWorld()
    exports["fa-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["fa-build"]:getModule("func").getCurrentBed()
...
function nearClothing()
    ...
    local isNear = exports["fa-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["fa-build"]:getModule("func").isInBuilding() then
...
RegisterInterfaceCallback("fa-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        exports["fa-interface"]:showContextMenu(MenuData["apartment_check"])
    end
end)
