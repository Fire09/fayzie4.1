...
AddEventHandler("playerConnecting", function(playerName, setKickReason, deferrals)
    ...
    deferrals.defer()
    deferrals.update(string.format("Hello %s. Checking ban status with NoProblem RP!", playerName))

    exports.oxmysql:execute("SELECT * FROM bans WHERE token = @token OR steam = @steam OR ip = @ip OR license = @rockstar", {
        ...
        if data ~= nil then
            if(data[1] == nil) then
                ...
                if data[1].expire == "Permenent" then
                    deferrals.done('\n You are banned, you may appeal it at https://discord.gg/pnp | Ban ID: ' .. data[1].banid .. ' | Reason: '..data[1].reason..' | Ban is Permenent.')
                elseif(os.time() > math.floor(tonumber(data[1].expire))) then
                    deferrals.done()
                    exports.oxmysql:execute('DELETE FROM bans WHERE steam = @steam', {['@steam'] = steamid})
                else
                    print(disp_time(tonumber(data[1].expire)).hours)
                    if disp_time(tonumber(data[1].expire)).hours < 24 then
                        deferrals.done('\n You are banned, you may appeal it at https://discord.gg/pnp | Ban ID: ' .. data[1].banid .. ' | Reason: '..data[1].reason..' | Ban expires in '..disp_time(tonumber(data[1].expire)).hours..' hours and '..disp_time(tonumber(data[1].expire)).minutes ..' minutes!')
                    else
                        deferrals.done('\n You are banned, you may appeal it at https://discord.gg/pnp | Ban ID: ' .. data[1].banid .. ' | Reason: '..data[1].reason..' | Ban expires in '..disp_time(tonumber(data[1].expire)).days..' days, '..disp_time(tonumber(data[1].expire)).hours..' hours and '..disp_time(tonumber(data[1].expire)).minutes ..' minutes!')
                    end
                end
            end
        end
