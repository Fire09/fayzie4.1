...
function SetRadioFrequency(pFrequency, pSendAppEvent)
    if CanUseFrequency(pFrequency, true) then
        ...
        -- exports["fa-ui"]:sendAppEvent("game", {
...
function SetRadioChannel(radioID)
    ...
    -- exports["fa-ui"]:sendAppEvent("game", { radioChannel = tostring(radioID) })

    Debug("[Radio] Channel Changed | Radio ID: %s", radioID)
end

function StartTransmission()
    if not IsRadioOn or not CurrentChannel or Throttled("radio:transmit") or exports['isPed']:isPed("dead") or IsTransmissionDisabled("radio") then return end
    ...
    if not IsTalkingOnRadio then
        ...
        TriggerEvent("fayzie:voice:transmissionStarted", { radio = true })
...
function LoadRadioModule()
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Push-To-Talk", "+transmitToRadio", "-transmitToRadio", "CAPITAL")
    RegisterCommand("+transmitToRadio", StartTransmission, false)
    RegisterCommand("-transmitToRadio", StopTransmission, false)

    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Push-To-Talk (Secondary)", "+secondaryTransmitToRadio", "-secondaryTransmitToRadio")
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Volume Up", "+increaseRadioVolume", "-increaseRadioVolume")
    RegisterCommand("+increaseRadioVolume", IncreaseRadioVolume, false)
    RegisterCommand("-increaseRadioVolume", function() end, false)

    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Volume Down", "+decreaseRadioVolume", "-decreaseRadioVolume")
    RegisterCommand("+decreaseRadioVolume", DecreaseRadioVolume, false)
    RegisterCommand("-decreaseRadioVolume", function() end, false)

    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "On / Off", "+toggleRadioState", "-toggleRadioState")
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Channel Up", "+increaseRadioChannel", "-increaseRadioChannel")
    RegisterCommand("+increaseRadioChannel", increaseRadioChannel, false)
    RegisterCommand("-increaseRadioChannel", function() end, false)

    exports["fa-keybinds"]:registerKeyMapping("", "Radio", "Channel Down", "+decreaseRadioChannel", "-decreaseRadioChannel")
    RegisterCommand("+decreaseRadioChannel", decreaseRadioChannel, false)
    RegisterCommand("-decreaseRadioChannel", function() end, false)

    RegisterNetEvent("fayzie:voice:radio:connect")
    AddEventHandler("fayzie:voice:radio:connect", ConnectToRadio)

    RegisterNetEvent("fayzie:voice:radio:disconnect")
    AddEventHandler("fayzie:voice:radio:disconnect", DisconnectFromRadio)

    RegisterNetEvent("fayzie:voice:radio:added")
    AddEventHandler("fayzie:voice:radio:added", AddRadioSubscriber) 

    RegisterNetEvent("fayzie:voice:radio:removed")
    AddEventHandler("fayzie:voice:radio:removed", RemoveRadioSubscriber)

    RegisterNetEvent("fayzie:voice:radio:power")
    AddEventHandler("fayzie:voice:radio:power", SetRadioPowerState)

    RegisterNetEvent("fayzie:voice:radio:volume")
    AddEventHandler("fayzie:voice:radio:volume", SetRadioVolume)
    ...
    TriggerEvent("fayzie:voice:radio:ready")
