...
function ConnectToDTFrequency(pIsHeadset)
    local subscribers = RPC.execute("fa-voice:drivethru:subscribe")
...
function DisconnectFromDTFrequency()
    ...
    RPC.execute("fa-voice:drivethru:unsubscribe")
    DTIsHeadset = false
...
function LoadDTModule()
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru", "-transmitToDriveThru")
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState", "-toggleDTRadioState")
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume", "-increaseDTVolume")
    RegisterCommand("+increaseDTVolume", IncreaseDTRadioVolume, false)
    RegisterCommand("-increaseDTVolume", function() end, false)

    exports["fa-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume", "-decreaseDTRadioVolume")
    ...
    RegisterNetEvent("fa-voice:drivethru:connect", ConnectToDTFrequency)

    RegisterNetEvent("fa-voice:drivethru:disconnect", DisconnectFromDTFrequency)

    RegisterNetEvent("fa-voice:drivethru:setPowerState", SetDTRadioPowerState)

    RegisterNetEvent("fa-voice:drivethru:addSubscriber", AddDTSubscriber)

    RegisterNetEvent("fa-voice:drivethru:removeSubscriber", RemoveDTSubscriber)

    -- scuffed way of overriding vanilla keybind
    AddEventHandler("fayzie:voice:transmissionStarted", function()
        if DTIsHeadset then return end
        StartDTTransmission()
    end)

    AddEventHandler("fayzie:voice:transmissionFinished", function()
        if DTIsHeadset then return end
        StopDTTransmission(true)
    end)
