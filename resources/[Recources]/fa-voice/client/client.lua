...
AddEventHandler("mumbleConnected", function()
    ...
    TriggerEvent("fayzie:voice:state", true)
end)

AddEventHandler("mumbleDisconnected", function()
    print("Mumble: Disconnected")
    TriggerEvent("fayzie:voice:state", false)
end)

AddEventHandler("fayzie:voice:state", function (state)
    VoiceEnabled = state

    TriggerServerEvent("fayzie:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("fayzie:voice:transmission:state", serverID, context, true)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("fayzie:voice:transmission:state", serverID, context, false)
        end
...
function AddGroupToTargetList(group, context, pIsInVehicle)
    ...
    TriggerServerEvent("fayzie:voice:transmission:state", group, context, true, nil, pIsInVehicle)
end
...
function RemoveGroupFromTargetList(group, context)
    ...
    TriggerServerEvent("fayzie:voice:transmission:state", group, context, false)
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("fayzie:voice:proximity", proximity)
    ...
    TriggerEvent("fa-hud:changeRange", range)
end
...
RegisterNetEvent("fayzie:voice:transmission:state")
AddEventHandler("fayzie:voice:transmission:state", function(serverID, context, transmitting, effect, isSourceInVehicle)
...
RegisterNetEvent("fayzie:voice:targets:player:add")
AddEventHandler("fayzie:voice:targets:player:add", AddPlayerToTargetList)

RegisterNetEvent("fayzie:voice:targets:player:remove")
AddEventHandler("fayzie:voice:targets:player:remove", RemovePlayerFromTargetList)

RegisterNetEvent("fayzie:voice:refresh")
AddEventHandler("fayzie:voice:refresh", RefreshConnection)

RegisterNetEvent("fayzie:voice:proximity:override")
AddEventHandler("fayzie:voice:proximity:override", function(pId, pMode, pRange, pPriority)
...
RegisterNetEvent("fa-voice:setTransmissionDisabled", function (pType, pDisabled)
...
Citizen.CreateThread(function()
    exports["fa-keybinds"]:registerKeyMapping("", "Voice", "Proximity / Range Toggle", "+cycleProximity", "-cycleProximity", "Z")
    RegisterCommand("+cycleProximity", CycleVoiceProximity, false)
    RegisterCommand("-cycleProximity", function() end, false)

    exports["fa-keybinds"]:registerKeyMapping("", "Voice", "Reset Mumble VOIP", "+mumble", "-mumble", "")
    ...
    TriggerEvent("fayzie:voice:ready")
...
RegisterNetEvent("fa-voice:setVoiceProximity")
AddEventHandler("fa-voice:setVoiceProximity", function(pProximity)
...
Citizen.CreateThread(function ()
    ...
    while true do
        ...
        if (isTalking and not isTalkingToggle) or IsTalkingOnRadio and not isTalkingOnRadio then
            ...
            TriggerEvent("fayzie:voice:transmissionStarted", { phone = true, radio = IsTalkingOnRadio })
        elseif (not isTalking and isTalkingToggle) or  isTalkingOnRadio and not IsTalkingOnRadio then
            ...
            TriggerEvent("fayzie:voice:transmissionFinished", { phone = false, radio = IsTalkingOnRadio })
        end
