...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports["fa-flags"]:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.flags = exports["fa-flags"]:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports["fa-flags"]:GetObjectFlags(pEntity)
    end
...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags["isJobEmployer"] then
        pContext.job = exports["fa-jobs"]:GetNPCJobData(npcId)
    end
end

--[[function isDisabled()
    return exports["fa-base"]:getVar("dead") or
        exports["fa-base"]:getVar("handcuffed")
end]]

function hasPhone()
    return exports["fa-inventory"]:hasEnoughOfItem("mobilephone", 1, false) or
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports["fa-vehicles"]:HasVehicleKey(pEntity)
...
function hasRepairItems(type)
    ...
    for i, v in pairs(items) do
        if exports["fa-inventory"]:hasEnoughOfItem(v, 1, false) then
            return true
        end
    end
...
AddEventHandler("fa-polyzone:enter", function(zone, data)
    if zone == "fa-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
AddEventHandler("fa-jobs:jobChanged", function(pJobId)
    CurrentJob = pJobId
