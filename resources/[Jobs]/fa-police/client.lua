...
Citizen.CreateThread(function()
    exports["fa-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
RegisterCommand('+stopEscort', function()
    job = exports['isPed']:isPed('job')
    if job == 'Police' then 
        TriggerEvent("drag:stopped")
    end
end, false)
RegisterCommand('+cuff', function()
    job = exports['isPed']:isPed('job')
    if job == 'Police' then
        TriggerEvent("fa-police:cuffFromMenu", false)
    end
end, false)
RegisterCommand('+uncuff', function()
    job = exports['isPed']:isPed('job')
    if job == 'Police' then
        TriggerEvent('fa-police:uncuffMenu')
    end
end, false)

RegisterCommand('-escort', function() end, false)
RegisterCommand('-stopEscort', function() end, false)
RegisterCommand('-cuff', function() end, false)
RegisterCommand('-uncuff', function() end, false)

Citizen.CreateThread(function()
    exports["fa-keybinds"]:registerKeyMapping("", "Police", "Escort", "+escort", "-escort", "")
    exports["fa-keybinds"]:registerKeyMapping("", "Police", "Stop Escort", "+stopEscort", "-stopEscort", "")
    exports["fa-keybinds"]:registerKeyMapping("", "Police", "Cuff", "+cuff", "-cuff", "")
    exports["fa-keybinds"]:registerKeyMapping("", "Police", "Un-Cuff", "+uncuff", "-uncuff", "")
end)

Citizen.CreateThread(function()
    while true do
        local coords = GetEntityCoords(PlayerPedId())
        TriggerServerEvent("fa-base:updatecoords", coords.x, coords.y, coords.z)
        Wait(13000)
    end
end)
...
AddEventHandler('event:control:police', function(useID)
    if useID == 1 then
        TriggerServerEvent('fa-police:checkForBar')

    elseif useID == 2 and isCop then
        TriggerEvent("server-inventory-open", "1", "evidenceLocker")
        TriggerServerEvent('fa-police:viewEvidenceLockup')
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'LockerOpen', 0.4)
...
RegisterNetEvent("fa-police:barCheck")
AddEventHandler("fa-police:barCheck", function(bar)
    if bar == 1 then
...
AddEventHandler("fa-jobmanager:playerBecameJob", function(job, name, notify)
    ...
    if job == "police" then
        isCop = true
        TriggerServerEvent('fa-police:getRank', "police")
        isInService = true
    end
    if job == "ems" then
        isMedic = true
        TriggerServerEvent('fa-police:getRank', "ems")
        isInService = true
    end
    if job == "doctor" then
        isDoctor = true
        TriggerServerEvent('fa-police:getRank', "doctor")
        isInService = true
    end
    if job == "news" then
        isNews = true
    ...
    while true do
        ...
        if recentsearch == 0 then
            ...
            if endDistance < 0.7 or endDistance2 < 0.7 then
                ...
                currentWeapons = exports["fa-inventory"]:GetCurrentWeapons()
...
AddEventHandler("isJudge", function()
    ...
    TriggerServerEvent('fa-police:getRank', "judge")
    isJudge = true
end)
...
RegisterNetEvent('fa-police:noLongerCop')
AddEventHandler('fa-police:noLongerCop', function()
    isCop = false
    isInService = false
    currentCallSign = ""
    ...
    TriggerServerEvent("fa-police:officerOffDuty")
...
-- RegisterNetEvent('fa-police:noLongerCop2')
-- AddEventHandler('fa-police:noLongerCop2', function()
...
-- TriggerServerEvent("myskin_customization:wearSkin")
-- TriggerServerEvent("fa-police:officerOffDuty2")
-- TriggerServerEvent('tattoos:retrieve')
-- TriggerServerEvent('Blemishes:retrieve')
-- RemoveAllPedWeapons(playerPed)
...
RegisterNetEvent('fa-police:checkPhone')
AddEventHandler('fa-police:checkPhone', function()
...
RegisterNetEvent('fa-police:checkLicenses')
AddEventHandler('fa-police:checkLicenses', function()
    t, distance = GetClosestPlayerIgnoreCar()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:getLicenses", GetPlayerServerId(t))
    else
...
RegisterNetEvent('fa-police:checkLicensePlate')
AddEventHandler('fa-police:checkLicensePlate', function(plate)
    if isCop then
...
RegisterNetEvent('fa-police:checkCrimes')
AddEventHandler('fa-police:checkCrimes', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:getCrimes", GetPlayerServerId(t))
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)

RegisterNetEvent('fa-police:deletecrimesciv')
AddEventHandler('fa-police:deletecrimesciv', function()
    if (isJudge) then
        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 7) then
            TriggerServerEvent("fa-police:deletecrimes", GetPlayerServerId(t))
        else
            TriggerEvent("DoLongHudText", "No player near you!", 2)
        end
    else
...
RegisterNetEvent('fa-police:checkBank')
AddEventHandler('fa-police:checkBank', function()
    t, distance, closestPed = GetClosestPlayer()
    if (distance ~= -1 and distance < 7) then
        TriggerServerEvent("fa-police:targetCheckBank", GetPlayerServerId(t))
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)

RegisterNetEvent('fa-police:checkInventory')
AddEventHandler('fa-police:checkInventory', function(isFrisk)
    if isFrisk == nil then
        isFrisk = false
    end
    t, distance, closestPed = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:targetCheckInventory", GetPlayerServerId(t), isFrisk)
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)

RegisterNetEvent("fa-police:rob")
AddEventHandler("fa-police:rob", function()
    ...
    if not exports["isPed"]:isPed("dead") then
        ...
        local finished = exports["fa-taskbar"]:taskBar(3500, "Robbing", false, true)
        ...
        if finished == 100 then
            ...
            if distance ~= -1 and distance < 5 and (IsEntityPlayingAnim(closestPed, "dead", "dead_d", 3) or
                ...
                ClearPedTasksImmediately(lPed)
                TriggerServerEvent("fa-police:rob", GetPlayerServerId(t))
                TriggerServerEvent('Gimmeurfuknmoney', GetPlayerServerId(t))
                TriggerServerEvent("fa-police:targetCheckInventory", GetPlayerServerId(t), false)
            else
                TriggerEvent("DoLongHudText", "No player near you!", 2)
            end
        end
...
RegisterNetEvent("fa-police:seizeCash") 
AddEventHandler("fa-police:seizeCash", function()

    t, distance, closestPed = GetClosestPlayer()

    if distance ~= -1 and distance < 5 then
        TriggerServerEvent("fa-police:SeizeCash", GetPlayerServerId(t))
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
...
RegisterNetEvent('fa-police:seizeInventory')
AddEventHandler('fa-police:seizeInventory', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:targetseizeInventory", GetPlayerServerId(t))
    else
...
AddEventHandler('KneelHU', function()
    ...
    if (DoesEntityExist(player) and not IsEntityDead(player)) then
        ...
        local finished = exports["fa-taskbar"]:taskBar(2500, "Surrendering")
...
RegisterNetEvent('fa-police:wshotgun')
AddEventHandler('fa-police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["fa-taskbar"]:taskBar(2000, "Unracking Shotgun")
            if finished == 100 then
...
RegisterNetEvent('fa-police:wrifle')
AddEventHandler('fa-police:wrifle', function()
    local targetVehicle = VehicleInFront()
    playerped = PlayerPedId()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
            local finished = exports["fa-taskbar"]:taskBar(2000, "Unracking Rifle")
            if finished == 100 then
...
RegisterNetEvent('fa-police:woxy')
AddEventHandler('fa-police:woxy', function()
    local vehFront = VehicleInFront()
    if vehFront > 0 then
        loadAnimDict('anim@narcotics@trash')
        TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 3800, 49, 3.0, 0, 0, 0)
        local finished = exports["fa-taskbar"]:taskBar(4000, "Grabbing Scuba Gear")
        if finished == 100 then
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
...
AddEventHandler('giveRadioToPed', function()
    ...
    if (not exports["fa-inventory"]:hasEnoughOfItem("radio", 1, false)) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["fa-taskbar"]:taskBar(2500, "Changing to Swat")
            if finished == 100 then
                ChangeToSkinNoUpdatePolice("s_m_y_swat_01")
            end
            ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('fa-police:fingerPrintPlayer')
AddEventHandler('fa-police:fingerPrintPlayer', function()
    local finished = exports["fa-taskbar"]:taskBar(8000,"DNA Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if(distance ~= -1 and distance < 5) then
            TriggerServerEvent("fa-police:dnaAsk", GetPlayerServerId(t))
        end
    end
end)
...
RegisterNetEvent('fa-police:freeze')
AddEventHandler('fa-police:freeze', function(ped)
    if (isCop) then
...
RegisterNetEvent('fa-police:remmaskAccepted')
AddEventHandler('fa-police:remmaskAccepted', function()
...
RegisterNetEvent('fa-police:remmask')
AddEventHandler('fa-police:remmask', function(t)
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        if isOppositeDir(GetEntityHeading(t), GetEntityHeading(PlayerPedId())) and
            ...
            TriggerServerEvent("fa-police:remmaskGranted", GetPlayerServerId(t))
...
RegisterNetEvent('fa-police:remweapons')
AddEventHandler('fa-police:remweapons', function(t)
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:remweaponsGranted", GetPlayerServerId(t))
    else
        TriggerEvent("DoLongHudText", "No player near you (maybe get closer)!", 2)
    end
end)
...
tryingcuff = false
RegisterNetEvent('fa-police:cuff2')
AddEventHandler('fa-police:cuff2', function(t, softcuff)
    if not tryingcuff then
        ...
        if (distance ~= -1 and #(GetEntityCoords(ped) - GetEntityCoords(PlayerPedId())) < 2.5 and GetEntitySpeed(ped) <
            1.0) then
            TriggerEvent('fa-police:cuff2')
            TriggerServerEvent("fa-police:cuffGranted2", GetPlayerServerId(t), softcuff)
        else
...
RegisterNetEvent('fa-police:cuff')
AddEventHandler('fa-police:cuff', function(t)
    if not tryingcuff then

        TriggerEvent("fa-police:ArrestingAnim")
        tryingcuff = true

        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 1.5) then
            TriggerServerEvent("fa-police:cuffGranted", GetPlayerServerId(t))
        else
            TriggerEvent("DoLongHudText", "No player near you (maybe get closer)!", 2)
        end
...
RegisterNetEvent('fa-police:cuffFromMenu')
AddEventHandler('fa-police:cuffFromMenu', function(softcuff)
    if not cuffstate and not handCuffed and not IsPedRagdoll(PlayerPedId()) and not IsPlayerFreeAiming(PlayerId()) and
        ...
        if (distance ~= -1 and distance < 2 and not IsPedRagdoll(PlayerPedId())) then
            ...
            TriggerEvent("fa-police:cuff2", GetPlayerServerId(t), softcuff)
        end
...
RegisterNetEvent('fa-police:gsr')
AddEventHandler('fa-police:gsr', function(t)
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_STAND_MOBILE", 0, 1)
    local finished = exports["fa-taskbar"]:taskBar(15000, "GSR Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 7) then
            TriggerServerEvent("fa-police:gsrGranted", GetPlayerServerId(t))
        end
    end
end)
...
RegisterNetEvent("fa-police:hasShotRecently")
AddEventHandler("fa-police:hasShotRecently", function(copId)
    TriggerServerEvent("fa-police:hasShotRecently", shotRecently, copId)
end)

RegisterNetEvent('fa-police:uncuffMenu')
AddEventHandler('fa-police:uncuffMenu', function()
    t, distance = GetClosestPlayer()
    -- error debug fix - syd
...
AddEventHandler('resetCuffs', function()
    ...
    TriggerEvent("fa-police:currentHandCuffedState", handCuffed, handCuffedWalking)
...
-- RegisterNetEvent('fa-police:getArrested2')
-- AddEventHandler('fa-police:getArrested2', function(cuffer)
...
-- local finished = 0
-- if not isDead then
-- finished = exports["fa-taskbarskill"]:taskBar(1200,7)
-- end
...
RPC.register("fa-police:canBeCuffed", function(pSource)
...
AddEventHandler('fa-police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        TriggerEvent("fa-police:recentlyAttemptedCuffed", GetGameTimer())
        ...
        finished = exports["fa-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
function CuffAnimation(cuffer)
    ...
    -- TriggerEvent('fa-police:cuffAttach',cuffer)
...
RegisterNetEvent('fa-police:cuffAttach')
AddEventHandler('fa-police:cuffAttach', function(cuffer)
    local count = 350
    while count > 0 do
        Citizen.Wait(1)
        count = count - 1
...
RegisterNetEvent('fa-police:cuffTransition')
AddEventHandler('fa-police:cuffTransition', function()
    loadAnimDict("mp_arrest_paired")
    Citizen.Wait(100)
...
RegisterNetEvent('fa-police:getArrested')
AddEventHandler('fa-police:getArrested', function(cuffer)
...
RegisterNetEvent('fa-police:jailing')
AddEventHandler('fa-police:jailing', function(args)
    Citizen.Trace("Jailing in process.")
    TriggerServerEvent('fa-police:jailGranted', args)
    TriggerServerEvent('updateJailTime', tonumber(args[2]))
    TriggerServerEvent('fa-jail:pJailedLog', tonumber(args[2]), tonumber(args[1]))
end)

RegisterNetEvent('fa-police:payFines')
AddEventHandler('fa-police:payFines', function(amount)
...
exports["fa-polyzone"]:AddBoxZone("evidence_zone", vector3(474.26, -1005.19, 26.27), 1, 3.0, {
    heading=0,
    --debugPoly=false,
    minZ=23.27,
    maxZ=27.27
...
AddEventHandler('fa-polyzone:enter', function(pZone)
    if pZone == "evidence_zone" then
        inEvidenceZone = true
        exports['fa-interface']:showInteraction('/evidence [LOCKER ID]')
    end
end)

RegisterNetEvent('fa-polyzone:exit')
AddEventHandler('fa-polyzone:exit', function(zoneName)
...
RegisterNetEvent('fa-police:forceEnter')
AddEventHandler('fa-police:forceEnter', function(id)
    ...
    if (distance ~= -1 and distance < 3) then
        ...
        if not isInVeh then
            ...
            TriggerServerEvent("fa-police:forceEnterAsk", GetPlayerServerId(t), netid)
...
RegisterNetEvent('fa-police:forcedEnteringVeh')
AddEventHandler('fa-police:forcedEnteringVeh', function(sender)
...
RegisterNetEvent('fa-police:tenThirteenA')
AddEventHandler('fa-police:tenThirteenA', function()
    exports["fa-dispatch"]:policedead('10-13A', "Officer Down URGENT", "630", true)
end)

RegisterNetEvent('fa-police:tenThirteenB')
AddEventHandler('fa-police:tenThirteenB', function()
    exports["fa-dispatch"]:policedead('10-13B', "Officer Down", "630", false)
end)

RegisterNetEvent("fa-police:tenForteenA")
AddEventHandler("fa-police:tenForteenA", function()
    exports["fa-dispatch"]:policedead('10-14A', "Medic Down URGENT", "630", true)
end)

RegisterNetEvent("fa-police:tenForteenB")
AddEventHandler("fa-police:tenForteenB", function()
    local pos = GetEntityCoords(PlayerPedId(), true)
    exports["fa-dispatch"]:policedead('10-14B', "Medic Down", "630", false)
end)

RegisterNetEvent("fa-police:setCallSign")
AddEventHandler("fa-police:setCallSign", function(pCallSign)
...
AddEventHandler('handCuffedWalking', function()
    ...
    if handCuffedWalking then
        ...
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'handcuff', 0.4)
        TriggerEvent("fa-police:currentHandCuffedState", false, false)
        return
    end
...
RegisterNetEvent('fa-police:Radio')
AddEventHandler('fa-police:Radio', function()
    if isCop then
...
function cuffCheck()
    if not handCuffed and not IsPedRagdoll(PlayerPedId()) and not IsPlayerFreeAiming(PlayerId()) and
        ...
        if (distance ~= -1 and distance < 3 and not IsPedRagdoll(PlayerPedId())) then
            TriggerServerEvent("fa-police:IsTargetCuffed", GetPlayerServerId(t))
        end
    end
end

RegisterNetEvent('fa-police:isPlayerCuffed')
AddEventHandler('fa-police:isPlayerCuffed', function(requestedID)
    TriggerServerEvent("fa-police:confirmIsCuffed", requestedID, handCuffed)
end)

RegisterNetEvent('fa-police:TargetIsCuffed')
AddEventHandler('fa-police:TargetIsCuffed', function(result)
    isTargetCuffed = result
    if isTargetCuffed then
        TriggerEvent("openSubMenu", "handcuffer")
    else
        TriggerEvent("fa-police:cuffFromMenu")
    end
    isTargetCuffed = false
end)

RegisterNetEvent('fa-police:AttemptCuffFromInventory')
AddEventHandler('fa-police:AttemptCuffFromInventory', function()
    cuffCheck()
end)
        ...
        if isCop and not inmenus then
            ...
            if isInVeh then
                ...
                if IsControlJustReleased(2, 172) and IsControlPressed(0, 19) then
                    TriggerEvent("fa-police:cuffFromMenu", true)
                    Citizen.Wait(400)
                end
                ...
                if IsControlJustReleased(2, 175) then
                    TriggerEvent("fa-police:forceEnter")
                    Citizen.Wait(400)
                end
                -- tackle
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            ...
            if myjob ~= "ems" and myjob ~= "doctor" then
                local bandages = exports["fa-inventory"]:getQuantity("bandage")
                if bandages == 0 then
                    return
                else
...
AddEventHandler("ems:stomachpump", function()
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            local finished = exports["fa-taskbar"]:taskBar(10000, "Inserting stomach pump 🤢", false, true)
...
AddEventHandler('checkmyPH', function()
    TriggerServerEvent("fa-police:showPH")
end)

RegisterNetEvent('fa-police:vin')
AddEventHandler('fa-police:vin', function()
    if isCop then
        playerped = PlayerPedId()
        coordA = GetEntityCoords(playerped, 1)
        coordB = GetOffsetFromEntityInWorldCoords(playerped, 0.0, 100.0, 0.0)
...
RegisterCommand('sc', function(source, args)
    TriggerServerEvent('fa-police:cuff2', source)
end)
...
AddEventHandler('unseatPlayer', function()
    ...
    if (distance ~= -1 and distance < 10) then
        ...
        TriggerServerEvent("fa-police:escortAsk", GetPlayerServerId(t))
    else
        TriggerEvent("DoLongHudText", 'No Player Found', 1)
    end
end)
...
AddEventHandler('dragPlayer', function()
    ...
    if (distance ~= -1 and distance < 1.0) then
        if not beingDragged then
            DetachEntity(PlayerPedId(), true, false)
            TriggerServerEvent("fa-police:dragAsk", GetPlayerServerId(t))
        end
    end
end)
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["fa-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
RegisterNetEvent("fa-police:chatCommand")
AddEventHandler("fa-police:chatCommand", function(args, cmd, isVehicleCmd)
    ...
    if isCop or isMedic then
        ...
        if cmd == "whitelist" then
            if rankService == 10 then
                ...
                TriggerServerEvent("fa-police:whitelist", arg2, arg)
            else
        ...
        if cmd == "remove" then
            if rankService == 10 then
                ...
                TriggerServerEvent("fa-police:remove", arg2, arg)
            else
                errorMsg("You do not have the rank to do this action")
            end
        end
...
RegisterCommand('callsigncar', function(source, args)
    if isCop then
        if args[1] == "plate" then
            ...
            TriggerEvent('fa-police:setCallSign', args[1])
        end
    else
...
RegisterCommand('livery', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'livery', true)
end)

RegisterCommand('extra', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'extra', true)
end)

RegisterCommand('tint', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'tint', true)
end)

RegisterCommand('clean', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'clean', true)
end)

RegisterCommand('color', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'color', true)
end)

RegisterCommand('color2', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'color2', true)
end)

-- RegisterCommand('whitelist', function(source, args)
--     TriggerEvent("fa-police:chatCommand", args, 'whitelist', false)
-- end)

RegisterCommand('remove', function(source, args)
    TriggerEvent("fa-police:chatCommand", args, 'remove', false)
end)

RegisterCommand('dorifto', function(source)
    TriggerEvent('fa-police:dorifto')
end)
...
local drift = false
RegisterNetEvent("fa-police:dorifto")
AddEventHandler("fa-police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["fa-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
RegisterNetEvent('fa-police:grantHouse')
AddEventHandler('fa-police:grantHouse', function()
    local status = 1
    local typeChange = "House"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        TriggerServerEvent("fa-police:updateLicenses", GetPlayerServerId(t), status, typeChange)
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)

RegisterNetEvent('fa-police:grantDriver')
AddEventHandler('fa-police:grantDriver', function()
    local status = 1
...
RegisterNetEvent('fa-police:grantBusiness')
AddEventHandler('fa-police:grantBusiness', function()
    local status = 1
...
RegisterNetEvent('fa-police:grantDA')
AddEventHandler('fa-police:grantDA', function()
    local status = 2
...
RegisterNetEvent('fa-police:removeDA')
AddEventHandler('fa-police:removeDA', function()
    local status = 0
    local typeChange = "Law"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:grantBar')
AddEventHandler('fa-police:grantBar', function()
    local status = 1
    local typeChange = "Law"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:removeBar')
AddEventHandler('fa-police:removeBar', function()
    local status = 0
    local typeChange = "Law"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:denyWeapon')
AddEventHandler('fa-police:denyWeapon', function()
    local status = 5
...
RegisterNetEvent('fa-police:denyDriver')
AddEventHandler('fa-police:denyDriver', function()
    local status = 5
    local typeChange = "Drivers"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:denyBusiness')
AddEventHandler('fa-police:denyBusiness', function()
    local status = 5
    local typeChange = "Business"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:denyHouse')
AddEventHandler('fa-police:denyHouse', function()
    local status = 5
    local typeChange = "House"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:grantWeapon')
AddEventHandler('fa-police:grantWeapon', function()
    local status = 1
    local typeChange = "Weapon"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:removeHouse')
AddEventHandler('fa-police:removeHouse', function()
    local status = 0
    local typeChange = "House"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:removeDriver')
AddEventHandler('fa-police:removeDriver', function()
    local status = 0
    local typeChange = "Drivers"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:removeBusiness')
AddEventHandler('fa-police:removeBusiness', function()
    local status = 0
    local typeChange = "Business"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:removeWeapon')
AddEventHandler('fa-police:removeWeapon', function()
    local status = 0
    local typeChange = "Weapon"
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
...
RegisterNetEvent('fa-police:frisk')
AddEventHandler('fa-police:frisk', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        local items = exports["fa-inventory"]:getFullItemList()
        TriggerServerEvent("fa-police:targetCheckWeapons", GetPlayerServerId(t),items)
        ExecuteCommand("e search")
        local finished = exports["fa-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
...
RegisterNetEvent('fa-police:hireNewPerson')
AddEventHandler('fa-police:hireNewPerson', function()
    exports['fa-interface']:openApplication('textbox', {
        callbackUrl = 'fa-police:hireNewPersonel',
        key = 1,
        items = {
          {
            icon = "id-card",
...
RegisterInterfaceCallback('fa-police:hireNewPersonel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local dept = data.values.department

    if dept == 1 then
        TriggerServerEvent('fa-fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('fa-fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('fa-fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)

-- Polyzone --

RegisterNetEvent('fa-police:hire')
AddEventHandler('fa-police:hire', function()
    TriggerServerEvent('fa-fa-police:sv:open_hire_menu')
end)

exports["fa-polytarget"]:AddCircleZone("faye_police_hire",  vector3(463.28, -984.29, 30.72), 0.55, {
    useZ = true
})

exports["fa-interact"]:AddPeekEntryByPolyTarget("faye_police_hire", {{
    event = "fa-police:hire",
    id = "faye_police_hire",
    icon = "id-badge",
    label = "Hire Personel!",
    parameters = {},
...
RegisterNetEvent('fa-police:BillLog')
AddEventHandler('fa-police:BillLog', function(pAmt, pPlayer)
    TriggerServerEvent('fa-jail:pBilledLog', pAmt, pPlayer)
end)
...
exports["fa-polyzone"]:AddBoxZone("faye_police_swat_armory", vector3(487.51, -997.09, 30.69), 1, 1.8, {
    heading=270,
    --debugPoly=false,
    minZ=27.09,
    maxZ=31.09
...
AddEventHandler('fa-polyzone:enter', function(name)
    if name == "faye_police_swat_armory" then
        if exports['isPed']:isPed('myjob') == "police" then
            pInSwatArmory = true
            exports["fa-interface"]:showInteraction("[E] Armory")
            SwatArmory()
        end
    end
end)
    ...
    Citizen.CreateThread(function()
      while pInSwatArmory do
        ...
        if IsControlJustReleased(0, 38) and exports['isPed']:isPed('myjob') == "police" then
            TriggerServerEvent('fa-police:openSwatArmory')
        end
      end
    end)
  end