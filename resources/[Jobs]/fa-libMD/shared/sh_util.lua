if not IsDuplicityVersion() then
    ...
    function SetCustomNuiFocus(hasKeyboard, hasMouse)
        ...
        TriggerEvent("fa-base:focus", HasNuiFocus, hasKeyboard, hasMouse)
    end
    ...
    function TaskItem(item, animDict, animName, typeAnim, speed, prop, message, timer, movement, weapon, remove)
        if not LocalPlayer.state.doge then
            TriggerEvent('fa-lib:forceAnimation', animDict, animName, typeAnim, speed, prop)
        end
        local finished = exports["fa-taskbar"]:TaskBar(message, timer, movement, weapon)
        if finished == 100 then
            if exports["fa-inventory"]:HasItem(item) then
                if remove then
                    TriggerEvent("inventory:destroy", item)
                end
                TriggerEvent('fa-lib:forceAnimation')
                return true
            else
    ...
    function GetNearbyPeds(X, Y, Z, Radius)
        ...
        if tonumber(X) and tonumber(Y) and tonumber(Z) then
            if tonumber(Radius) then
                for Ped in exports["fa-admin"]:EnumeratePeds() do
    ...
    function SkillCircleLooped(count)
        ...
        while loop < count do
            loop = loop + 1
            local finished = exports["fa-skillcircle"]:SkillCircle(math.random(13, 60))
    ...
    function SpeedSkillCircle()
        local finished = exports["fa-skillcircle"]:SkillCircle(math.random(4, 7))
        if not finished then
            return false
        end
        return true
    ...
    function SkillCircleLooped2(sent)
        ...
        for i = 1, #speeds do
            local finished = exports["fa-skillcircle"]:SkillCircle(speeds[i])
            if not finished then
                return false
            end
            Wait(200)
    ...
    function SpawnVehicle(model, spawn, heading)
        ...
        TriggerEvent('fa-vmod:addKeys', _vehicle)
