local activeRegisters = {
    ...
    {
        polytarget = {
            vector3(-585.26, -609.66, 34.68), 0.6, 0.6, {
                heading=5,
                minZ=34.48,
                maxZ=35.28
            }
    ...
    {
        polytarget = {
            vector3(-585.23, -613.4, 34.68), 0.6, 0.6, {
                heading=5,
                minZ=34.68,
                maxZ=35.28
            }
...
for id,register in ipairs(activeRegisters) do
        ...
        exports["fa-polytarget"]:AddBoxZone(ptId, register.polytarget[1], register.polytarget[2], register.polytarget[3], register.polytarget[4])
        exports['fa-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "fa-business:registerPurchasePrompt",
            id = ptId .. "_purchase",
            icon = "cash-register",
            label = "make payment",
        ...
        exports['fa-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "fa-business:registerChargePrompt",
            id = ptId .. "_charge",
            icon = "credit-card",
...
AddEventHandler('fa-business:registerPurchasePrompt', function(pParameters, pEntity, pContext)
    ...
    local acceptContext = {
        ...
        },
        {
            ...
            action = "fa-business:finishPurchasePrompt",
            icon = 'credit-card',
    ...
    exports['fa-interface']:showContextMenu(acceptContext)
end)

AddEventHandler('fa-business:registerChargePrompt', function(pParameters, pEntity, pContext)
    exports['fa-interface']:openApplication('textbox', {
        callbackUrl = 'fa-interface:business:charge',
...
RegisterNetEvent('fa-business:activePurchase')
AddEventHandler("fa-business:activePurchase", function(data)
    activePurchases[data.registerId] = data
end)

--Remove at registerId pos
RegisterNetEvent('fa-business:closePurchase')
AddEventHandler("fa-business:closePurchase", function(data)
    activePurchases[data.registerId] = nil
end)

RegisterInterfaceCallback('fa-business:finishPurchasePrompt', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local success, message = RPC.execute("fa-business:completePurchase", data.key)  
    if not success then
        TriggerEvent("DoLongHudText", message, 2)
    end 
end)

RegisterInterfaceCallback("fa-interface:business:charge", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    RPC.execute("fa-business:startPurchase", {cost = cost, comment = comment, registerId = data.key.registerId, business = data.key.business})
end)