...
function policeCuff()
	if not inmenus and (isCop or isState or isSheriff) and not IsPauseMenuActive() then
		local isInVeh = IsPedInAnyVehicle(PlayerPedId(), false)
		if isInVeh then
...
function emsHeal()
	if not inmenus and (isMedic or isDoctor or isDoc or isCop) and not IsPauseMenuActive() then
...
Citizen.CreateThread(function()
    ...
    exports["fa-keybinds"]:registerKeyMapping("", "Vehicle", "Cruise Control", "+vehCruise", "-vehCruise", "X")

    RegisterCommand('+policeCuff', policeCuff, false)
	RegisterCommand('-policeCuff', function() end, false)
    exports["fa-keybinds"]:registerKeyMapping("", "Gov", "Cuff / Radar Front", "+policeCuff", "-policeCuff", "UP")

    RegisterCommand('+policeUnCuff', policeUnCuff, false)
	RegisterCommand('-policeUnCuff', function() end, false)
    exports["fa-keybinds"]:registerKeyMapping("", "Gov", "UnCuff / Radar Rear", "+policeUnCuff", "-policeUnCuff", "DOWN")

    RegisterCommand('+policeSoft', policeSoft, false)
	RegisterCommand('-policeSoft', function() end, false)
    exports["fa-keybinds"]:registerKeyMapping("", "Gov", "Soft Cuff", "+policeSoft", "-policeSoft")

    RegisterCommand('+policeSeat', policeSeat, false)
	RegisterCommand('-policeSeat', function() end, false)
	exports["fa-keybinds"]:registerKeyMapping("", "Gov", "Force into Vehicle", "+policeSeat", "-policeSeat", "RIGHT")

    RegisterCommand('+policeEscort', policeEscort, false)
	RegisterCommand('-policeEscort', function() end, false)
	exports["fa-keybinds"]:registerKeyMapping("", "Gov", "Escort / Speedo", "+policeEscort", "-policeEscort", "LEFT")

    RegisterCommand('+emsHeal', emsHeal, false)
	RegisterCommand('-emsHeal', function() end, false)
	exports["fa-keybinds"]:registerKeyMapping("", "Gov", "EMS Heal", "+emsHeal", "-emsHeal")

    RegisterCommand('+medicRevive', medicRevive, false)
	RegisterCommand('-medicRevive', function() end, false)
	exports["fa-keybinds"]:registerKeyMapping("", "Gov", "EMS Revive", "+medicRevive", "-medicRevive")
...
RegisterInterfaceCallback('police:hireNewPersonel', function(data, cb)
    ...
    if dept == 1 then
        TriggerServerEvent('fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('fa-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)
...
AddEventHandler('police:hire', function()
    TriggerServerEvent('fa-police:sv:open_hire_menu')
end)
