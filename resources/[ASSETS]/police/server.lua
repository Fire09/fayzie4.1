...
function CheckLicense(cid, license)
    exports.oxmysql:execute("SELECT @license FROM licenses WHERE cid = @id", {
        ['id'] = cid,
        ['license'] = license
    }, function(result)
        if (result[1]) then
...
RegisterCommand('callsign', function(source, args)
    ...
    exports.oxmysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character.id}, function(result)
        if result[1].job == 'police' or result[1].job == 'ems' then
            print("HELLO COCK")
            exports.oxmysql:execute("UPDATE jobs_whitelist SET `callsign` = @callsign WHERE cid = @cid", {
                ['callsign'] = args[1],
                ['cid'] = character.id
            })
...
function checkDBForDna(ident, dna)
    local canUse = true
    exports.oxmysql:execute("SELECT meta FROM characters WHERE id = @identifier", {
        ['identifier'] = ident
    }, function(result)
        if (result[1]) then
...
AddEventHandler('bones:server:updateServer', function(bones)
    ...
    exports.oxmysql:execute("UPDATE characters SET `bones` = @bones WHERE id = @identifier", {
        ['bones'] = bones,
...
AddEventHandler("Evidence:checkDna", function()
    ...
    exports.oxmysql:execute("SELECT dna FROM characters WHERE id = @identifier", {
        ['identifier'] = char.id
    }, function(result)
        if (result[1]) then
    ...
    if needsNewDna then
        ...
        exports.oxmysql:execute("UPDATE characters SET `dna` = @metaData WHERE id = @identifier", {
            ['metaData'] = meta,
            ['identifier'] = characterId
        })
    end
...
AddEventHandler('police:whitelist', function(arg, jobType)
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.oxmysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ['job'] = jobType,
            ["cid"] = arg
        }, function(result)
            if (result[1]) then
    ...
    elseif jobType == "judge" then
        exports.oxmysql:execute("SELECT judge_type FROM characters WHERE id = @cid", {
            ["cid"] = char.id
        }, function(result)
            if result[1] then
                if result[1].judge_type == 10 then
                    ...
                    exports.oxmysql:execute("UPDATE characters SET `judge_type`=@rank WHERE id = @cid", {
                        ['rank'] = 1,
                        ["cid"] = arg
                    })
                    TriggerClientEvent("DoLongHudText", src,
...
function jailUser(player, reason, cid, time, src)
    ...
    TriggerEvent("fa-police:playerJailed", character)
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    ...
    exports.oxmysql:execute('SELECT * FROM user_inventory2 WHERE `name` = @name', { ['@name'] = pName }, function(result)
        if result[1] ~= nil then
...
RegisterServerEvent('fa-police:hire_pd')
AddEventHandler('fa-police:hire_pd', function(pCID, pRank, pCallsign, pDept)
...
RegisterServerEvent('fa-police:sv:open_hire_menu')
AddEventHandler('fa-police:sv:open_hire_menu', function()
    local src = source
    local user = exports["fa-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
...
AddEventHandler('police:openSwatArmory', function()
    ...
    exports.oxmysql:execute('SELECT rank FROM jobs_whitelist WHERE cid = ?', {character.id}, function(result)
        print('Rank: ' .. result[1].rank)
        if result[1].rank >= 7 then
			TriggerClientEvent('server-inventory-open', src, '10', 'Shop')
		else
			TriggerClientEvent("DoLongHudText", src,"You are not a high enough rank to use this!", 2)
		end
	end) 
