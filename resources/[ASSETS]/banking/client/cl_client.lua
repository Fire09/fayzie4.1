...
Citizen.CreateThread(function()
	...
	exports["fa-polyzone"]:AddBoxZone("wakingup", vector3(316.09, -582.32, 43.28), 17.0, 7.4, {
    name="wakingup",
		heading=359.12,
		debugPoly=false,
		minZ=36.99,
...
AddEventHandler('fa-polyzone:enter', function(name)
    if name == "banking" then
        banking = true
        exports["fa-interface"]:showInteraction("[E] Banking")
        bankingopen()
    end
end)
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["fa-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(PlayerPedId())
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["fa-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(PlayerPedId())
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
...
RegisterNetEvent("fa-base:addedMoney")
AddEventHandler("fa-base:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("fa-base:removedMoney")
AddEventHandler("fa-base:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
