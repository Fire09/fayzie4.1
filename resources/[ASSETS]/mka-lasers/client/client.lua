...
local function calculateCurrentPoint(fromPoint, toPoint, deltaTime, travelTimeBetweenTargets)
  local FayedDirection = toPoint - fromPoint
  local FayedDirectionDist = #FayedDirection
  local percentOfTravelTime = deltaTime / (travelTimeBetweenTargets * 1000)
  local distance = math.min(FayedDirectionDist * percentOfTravelTime, FayedDirectionDist)
  return fromPoint + (norm(FayedDirection) * distance)
end
