...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsEntityDead(player) then
            if exports['fa-admin']:IsInGodMode() then
                local ped = PlayerPedId()
                TriggerEvent("Heal")
                SetMaxHealth()
                SetMaxArmor()
            ...
            -- local isHardcore = exports['fa-character']:GetCharacterType() == 'hardcore'
            ...
            --         score = score * 2
            --     end
            --     exports['fa-character']:AddHCScore(score)

            --     local icu = exports['fa-character']:RollICU()
            --     Wait(1000)
            --     if icu then
...
function getDeadAnimTreeAndAnim()
    ...
    local _, isAnimalModel = pcall(function () return exports["fa-misc-code"]:isAnimalModel(model) end)
    if not isAnimalModel then return defaultAnimTree, defaultAnim end

    local _, animTree = pcall(function () return exports["fa-misc-code"]:getAnimalModelDict(model) end)
    local _, anim = pcall(function () return exports["fa-misc-code"]:getAnimalModelAnim(model, "dead") end)
...
AddEventHandler("fa-menu:var:inServerFarm", function(pBool)
...
AddEventHandler('trycpr', function()
    ...
    if curDist < 5 or curDist2 < 5 then
        ...
        while penis < 10 do
            penis = penis + 1
            local finished = exports["fa-ui"]:taskBarSkill(math.random(2000, 6000), math.random(5, 10))
...
function releaseBody(dropItems)
    ...
    local doctors = RPC.execute('fa-jobmanager:jobCount', 'doctor')
    ...
    TriggerServerEvent('fa-healthcare:playerRespawn', hosp)
