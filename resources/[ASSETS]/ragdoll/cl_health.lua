...
function OnDeath(pMinor)
    ...
    local newDeaths = RPC.execute('fa-ragdoll:addDeath', pMinor)
    SetDeaths(newDeaths)
end
...
AddEventHandler('fa-spawn:characterSpawned', function()
    local deaths = RPC.execute('fa-ragdoll:fetchDeaths')
    SetDeaths(deaths)
end)

RegisterNetEvent('fa-ragdoll:refreshDeaths', function(deaths)
    SetDeaths(deaths)
end)
