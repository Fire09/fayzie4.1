...
RegisterNetEvent('fa-base:PolyZoneUpdate')
AddEventHandler('fa-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "fa-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('fa-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('fa-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('fa-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('fa-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('fa-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('fa-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('fa-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('fa-polyzone:stop')
            end
        end
