...
RegisterNetEvent("fa-inventory:itemCheck", function(item, hasItem, quantity)
    ...
    if not hasItem then
        if item == "boxinggloves" then
            ...
            TriggerEvent("fa-inventory:boxingGlovesEquipped", false)
        end
        SetDefaultValues()
        return
    end
    ...
    for buff, value in pairs(buffs) do
        ...
        if buff == "meleedamage" then
            ...
            if item == "boxinggloves" then
                ...
                TriggerEvent("fa-inventory:boxingGlovesEquipped", true)
...
Citizen.CreateThread(function()
    ...
    local buffsEnabled = exports["fa-config"]:GetMiscConfig("buffs.enabled")
