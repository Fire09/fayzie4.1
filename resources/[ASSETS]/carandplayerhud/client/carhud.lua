...
toggleFaceWear = function(pWearType, pShouldRemove)
  ...
  if pWearType == 1 and not pShouldRemove and removedHatManually then
    local hasHat = exports["fa-inventory"]:hasEnoughOfItem("hat", 1, false, true, {hat = facialWear[PropIndex+1]["Prop"]})
    if not hasHat then
  ...
  if pWearType == 4 and not pShouldRemove and removedMaskManually then
    local hasMask = exports["fa-inventory"]:hasEnoughOfItem("mask", 1, false, true, {mask = facialWear[pWearType]["Prop"]})
    if not hasMask then
    ...
    if pWearType ~= 5 then
      ...
      if pWearType == 4 then
        ...
        if currentDrawable ~= -1 then
          ...
          local hasPropItem = exports["fa-inventory"]:hasEnoughOfItem("mask", 1, false, true, itemMeta)
        ...
        if pWearType ~= 2 then
          ...
          if pWearType == 1 and currentProp ~= -1 then
            ...
            local hasPropItem = exports["fa-inventory"]:hasEnoughOfItem("hat", 1, false, true, itemMeta)
            if not hasPropItem then
...
AddEventHandler("fa-jobmanager:playerBecameJob", function(job, name)
  ...
  if DoesEntityExist(objFound) then
    -- TriggerEvent("DoLongHudText","Press [E] to leave the bed at any time.",1)
      ...
      if not disableKeys then
        showInteraction = true
        exports["fa-ui"]:showInteraction("[E] Leave Bed; [F] Dismiss")
      end
...
Citizen.CreateThread(function()
  while true do
    local isSitting = exports["fa-flags"]:HasPedFlag(PlayerPedId(), "isSittingOnChair")
...
RegisterNetEvent('fa-voice:focus:set')
AddEventHandler('fa-voice:focus:set', function(hasFocus, hasKeyboard, hasMouse)
...
RegisterNetEvent('pnp:admin:maxstats')
AddEventHandler('pnp:admin:maxstats', function()
    currentValues["thirst"] = 100
    currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("fa-death:reviveSV", source)
    ...
    exports['ragdoll']:SetPlayerArmor(200)
      ...
      if GetGameTimer() - lastDamageTrigger > 3000 then
        ...
        if currentValues["oxy"] <= 0 then
          ...
          if exports["isPed"]:isPed("dead") then
            ...
            -- currentValues["health"] = GetEntityHealth(plyPed) - 20
          end
        end
      end
    end
...
function initHud(hasStarted)
  ...
  if hasStarted then
    --Wait(100)
    local initialHudSettings = exports["fa-hud"]:GetPreferences()
    if initialHudSettings ~= "{}" then
      exports["fa-hud"]:sendAppEvent("preferences", initialHudSettings)
      exports["fa-ui"]:sendAppEvent('preferences', initialHudSettings)
      TriggerEvent("fa-preferences:setPreferences", initialHudSettings)
    end
...
function getArmorAndStressData()
  ...
  if meta.health < 110.0 then
    ...
    currentValues["health"] = exports['ragdoll']:SetPlayerHealth(meta.health)
  end
...
Citizen.CreateThread(function()
  ...
  local thespeed = math.ceil(GetEntitySpeed(get_ped_veh) * 3.6)
  ...
  while true do
    ...
    if counter == 0 then
      ...
      thespeed = math.ceil(GetEntitySpeed(get_ped_veh) * 3.6)
      ...
      if valueChanged then
        -- RPC.execute("fa-hud:UpdateNeeds", currentValues)
        exports["fa-hud"]:startHealthArmorUpdates()
        exports["fa-hud"]:sendAppEvent("hud", {
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    local hasCrosshair =  IsPedArmed(PlayerPedId(), 6) and IsPlayerFreeAiming(PlayerId()) and not exemptHashes[GetSelectedPedWeapon(PlayerPedId())]
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("fa-buffs:addUIBuff", { buffedHunger = true, buffedThirst = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 6hr
    buffStartTime = 0
    TriggerEvent("fa-buffs:addUIBuff", { buffedHunger = false, buffedThirst = false })
  end)
end)
...
AddEventHandler("buffs:triggerBuff", function(item)
  ...
  TriggerEvent("fa-buffs:addUIBuff", { buffedHealth = true, buffedArmor = true })
  Citizen.CreateThread(function()
    Citizen.Wait(60000 * 60 * 6) -- 3hr
    citrineStartTime = 0
    TriggerEvent("fa-buffs:addUIBuff", { buffedHealth = false, buffedArmor = false })
  end)
end)
Citizen.CreateThread(function()
  while true do
...
AddEventHandler("fa-inventory:itemUsed", function(item)
  ...
  local finished = exports["fa-taskbar"]:taskBar(4000, "Using Chapstick")
...
Citizen.CreateThread(function()
  while true do
    ...
    if currentValues["thirst"] < 20 or currentValues["hunger"] < 20 then
      ...
      -- currentValues["health"] = newhealth
    end
    ...
    if currentValues["thirst"] < 2 or currentValues["hunger"] < 2 then
      ...
      -- currentValues["health"] = math.max(newhealth, 1)
    end
...
AddEventHandler("fa-police:cuffs:state", function(pIsHandcuffed, pIsHandcuffedAndWalking)
...
AddEventHandler("baseevents:enteredVehicle", function(pVehicle)
  ...
  local hasLicense = exports["fa-gov"]:HasLicense("pilot_license")
  if hasLicense then
    return
  end
