...
AddEventHandler('jail:charecterFullySpawend', function()
    ...
    exports.oxmysql:execute("SELECT * FROM `characters` WHERE id = @cid", {['cid'] = character.id}, function(result)
...
AddEventHandler("checkJailTime", function(sendmessage)
    ...
    exports.oxmysql:execute("SELECT * FROM `characters` WHERE id = @cid", {['cid'] = char.id}, function(result)
...
AddEventHandler("jail:cuttime", function()
    ...
    exports.oxmysql:execute("SELECT * FROM `characters` WHERE id = @cid", {['cid'] = char.id}, function(data)
        ...
        exports.oxmysql:execute("UPDATE `characters` SET `jail_time` = @time WHERE `id` = @cid", {['time'] = recent - tonumber(5), ['cid'] = char.id})
    end)
...
--     local user = exports["fa-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     local target = exports["fa-base"]:getModule("Player"):GetUser(tonumber(args[1]))

--     if user:getVar("job") == 'police' or user:getVar("job") == 'doc' then
--         if tonumber(args[1]) and exports["fa-base"]:getModule("Player"):GetUser(tonumber(args[1])) then
