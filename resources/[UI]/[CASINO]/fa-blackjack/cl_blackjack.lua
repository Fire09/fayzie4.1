...
cfg.blackjackTables = {
    --[id] = {x,y,z,heading}
    [0] = {
        dealerPos = vector3(1044.177,53.479,69.060),
        dealerHeading = 195.867,
        tablePos = vector3(1044.392, 52.664, 68.060),
        tableHeading = 13.465,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [1] = {
        dealerPos = vector3(1036.622, 51.720, 69.060),
        dealerHeading = 192.866,
        tablePos = vector3(1036.813, 50.905, 68.060),
        tableHeading = 15.156,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [2] = {
        dealerPos = vector3(1036.066, 54.218, 69.060),
        dealerHeading = 15.515,
        tablePos = vector3(1035.85, 55.024, 68.060),
        tableHeading = 193.156,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01"
    },
    [3] = {
        dealerPos = vector3(1043.589, 55.954, 69.060),
        dealerHeading = 12.758,
        tablePos = vector3(1043.424, 56.797, 68.060),
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        exports["fa-ui"]:showInteraction("Press [E] to play blackjack", 'info')
                    else 
                        exports["fa-ui"]:showInteraction("This seat is taken", 'danger')
                        Citizen.Wait(3000)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            exports["fa-ui"]:showInteraction("Your hand is["..tostring(currentHand).."], the dealers hand is ["..tostring(dealersHand).."]")
    ...
    while true do
        if closestChair ~= -1 and closestChairDist < 2 then 
            if IsControlJustPressed(0, 38) then --Sit down [E]
                ...
                if exports["fa-inventory"]:hasEnoughOfItem("casinomember",1,false) then
                    if blackjackTableData[closestChair] == false then
                        TriggerServerEvent("Blackjack:requestSitAtBlackjackTable",closestChair)
                    end
                else 
                    TriggerEvent('DoLongHudText', "You Are Not A Member!!", 2)  
                end
            end
        end
        Wait(0)
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            TriggerEvent("fa-context:closemenu")
        end
    end)
end)
...
AddEventHandler("betmenu", function()
    local tmpInput = exports["fa-applications"]:KeyboardInput({
        header = "Bet Amount",
        rows = {
            {
                id = 1,
