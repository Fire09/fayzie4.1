...
function setNuiActive(booleanOrNil)
  ...
  if boolean ~= isNuiActive then
    if boolean then TriggerServerEvent('fayesql:request-data') end
    isNuiActive = boolean
...
CreateThread(function()
  if isNuiActive then TriggerServerEvent('fayesql:request-data') end
  Wait(300000)
end)
...
RegisterNetEvent('fayesql:update-resource-data')
AddEventHandler('fayesql:update-resource-data', function (resourceData)
...
RegisterNetEvent('fayesql:update-time-data')
AddEventHandler('fayesql:update-time-data', function (timeData)
  local timeArray = {}
...
RegisterNetEvent('fayesql:update-slow-queries')
AddEventHandler('fayesql:update-slow-queries', function(slowQueryData)
...
RegisterNetEvent('fayesql:update-status')
AddEventHandler('fayesql:update-status', function(statusData)
  SendNUIMessage({
    type = 'onStatusData',
    status = statusData,
  });
...
RegisterNetEvent('fayesql:update-variables')
AddEventHandler('fayesql:update-variables', function(variableData)
  SendNUIMessage({
    type = 'onVariableData',
...
RegisterNUICallback('request-server-status', function()
  TriggerServerEvent('fayesql:request-server-status')
end)

RegisterNetEvent('fayesql:get-table-schema')
AddEventHandler('fayesql:get-table-schema', function(tableName, schema)
  SendNUIMessage({
    type = 'onTableSchema',
    tableName = tableName,
    schema = schema,
...
RegisterNUICallback('request-table-schema', function(tableName)
  TriggerServerEvent('fayesql:request-table-schema', tableName)
end)
