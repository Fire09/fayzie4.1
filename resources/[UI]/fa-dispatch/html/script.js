...
function setCoords(x, y) {
    $.post('https://fa-dispatch/sfa-coords', JSON.stringify({
        x: x,
        y: y
    }));
}
...
$(document).ready(function() {
	window.addEventListener('message', function(event){
		if (event.data.action == "open") {
            ...
            $('.mesai-notifa-main').attr('saveattr', "1");
            $('.police-list').html("");
            $('.police-list').html("");

            $('.out-dispatch-notifys').html("");
            $('.dispatch-menu').html("");
            $('.marked-notifa-main').html("");
            ...
            <li class="MuiButtonBase-root MuiListItem-root MuiMenuItem-root MuiMenuItem-gutters MuiListItem-gutters MuiListItem-button sfa-gps" tabindex="-1" role="option" aria-disabled="false" data-value="10">
            Set GPS Location<span class="MuiTouchRipple-root">
            </span></li>`)
            $('.sfa-gps').click(function() {
                $.post('https://fa-dispatch/sfa-coords', JSON.stringify({
                    number: roninNumber
                }));
                $('.called-list').fadeOut();
            ...
            $('.sfa-gps-location').click(function() {
                $.post('https://fa-dispatch/sfa-coords', JSON.stringify({
                    number: roninNumber
                }));
        ...
        if (event.data.action == "update-html") {
            if (event.data.code) {
                ...
                $('.mesai-notifa-main').each(function(){
            ...
            // $(`.mesai-notifa-main[saveattr = "1"]`).remove();
		}
        ...
        if (event.data.action == "close") {
            $.post('https://fa-dispatch/close', JSON.stringify({}));
            outKapa();
		}

        if (event.data.action == "close-callsign") {

            $.post('https://fa-dispatch/close-callsign', JSON.stringify({}));
            outKapa();
		}
        ...
        if (event.data.action == "create_call") {
            ...
            $('.marked-notifa-main').prepend(event.data.call);
        ...
        $('.create-call').click(function() {
            $.post('https://fa-dispatch/create_call', JSON.stringify({
                number: roninNumber
            }));
        })
        ...
        $('.dismiss-pingg').click(function() {
            createCall3.addClass('dismissed');
            $('.called-list').fadeOut();
            $.post('https://fa-dispatch/dismiss_call-2', JSON.stringify({
                number: roninNumber
            }));
        })
    ...
    function createNotify(coordx, coordy, greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        if (car == "" || car == undefined) {
            ...
            carCheck = `<div class="notifa-divs">
        ...
        html += `<div class="notifa-main-container-inner" number="${greenNumber}">
			<div class="notifa-border notifa-animate" aria-controls="ping-menu-263" aria-haspopup="true" x="${coordx}" y="${coordy}">
			   <div class="notifa-divs">
				  <div class="notifa-header">
				  ...
				  <div class="notifa-header-icon gps-icon">
         ...
         setTimeout(function () {
            $(`.notifa-main-container-inner[number="${greenNumber}"`).css('animation', 'notifyRevers 0.5s');
            setTimeout(function(){
                $(`.notifa-main-container-inner[number="${greenNumber}"`).remove();
            },490)
         },5000)
        return html;
    }
    ...
    function createNotify2(coordx, coordy, greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        html += `<div class="notifa-main-container-inner" number="${greenNumber}">
			<div class="notifa-border notifa-animate" aria-controls="ping-menu-263" aria-haspopup="true"  x="${coordx}" y="${coordy}" style="background-color: rgba(120, 15, 0, 0.824); border-right: rgb(231,55,24)">
			   <div class="notifa-divs">
				  <div class="notifa-header">
					 <div class="MuiChip-root MuiChip-colorPrimary MuiChip-sizeSmall"><span class="MuiChip-label MuiChip-labelSmall">${greenNumber}</span></div>
					 <div class="MuiChip-root MuiChip-colorSecondary MuiChip-sizeSmall" style="margin-left: 8px;"><span class="MuiChip-label MuiChip-labelSmall">${orangeNumber}</span></div>
    ...
    function createMenuNotifypolice(coordx, coordy ,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        html += `<div class="notifa-border" aria-controls="ping-menu-263" aria-haspopup="true" x="${coordx}" y="${coordy}" style="background-color: rgba(120, 15, 0, 0.824); border-right: rgb(231,55,24)">
			<div class="notifa-divs">
			   <div class="notifa-header">
				  <div class="MuiChip-root MuiChip-colorPrimary MuiChip-sizeSmall"><span class="MuiChip-label MuiChip-labelSmall">${greenNumber}</span></div>
				  <div class="MuiChip-root MuiChip-colorSecondary MuiChip-sizeSmall" style="margin-left: 8px;"><span class="MuiChip-label MuiChip-labelSmall">${orangeNumber}</span></div>
    ...
    function createMenuNotify(coordx, coordy ,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        html += `<div class="notifa-border" aria-controls="ping-menu-263" aria-haspopup="true" x="${coordx}" y="${coordy}">
			<div class="notifa-divs">
			   <div class="notifa-header">
				  <div class="MuiChip-root MuiChip-colorPrimary MuiChip-sizeSmall"><span class="MuiChip-label MuiChip-labelSmall">${greenNumber}</span></div>
				  <div class="MuiChip-root MuiChip-colorSecondary MuiChip-sizeSmall" style="margin-left: 8px;"><span class="MuiChip-label MuiChip-labelSmall">${orangeNumber}</span></div>
    ...
    function createMenuNotifyMod2(coordx, coordy ,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        if (car == "" || car == undefined) {
            ...
            carCheck = `<div class="notifa-divs called">
				<i class="fa-solid fa-car-side fa-w-20 fa-fw"></i>
				<p class="MuiTypography-root MuiTypography-body1 MuiTypography-colorTextPrimary" style="word-break: break-word;">${car}</p>
				<i class="fa-solid fa-closed-captioning fa-w-16 fa-fw"></i>
				<p class="MuiTypography-root MuiTypography-body1 MuiTypography-colorTextPrimary" style="word-break: break-word;">${plate}</p>
        ...
        html += `<div class="notifa-border called" aria-controls="ping-menu-263" aria-haspopup="true" x="${coordx}" y="${coordy}">
			<div class="notifa-divs">
			   <div class="notifa-header">
				  <div class="MuiChip-root MuiChip-colorPrimary MuiChip-sizeSmall"><span class="MuiChip-label MuiChip-labelSmall">${greenNumber}</span></div>
				  <div class="MuiChip-root MuiChip-colorSecondary MuiChip-sizeSmall" style="margin-left: 8px;"><span class="MuiChip-label MuiChip-labelSmall">${orangeNumber}</span></div>
    ...
    function createMenuNotifyMod3(coordx, coordy ,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        ...
        if (car == "" || car == undefined) {
            ...
            carCheck = `<div class="notifa-divs called dismissed">
				<i class="fa-solid fa-car-side fa-w-20 fa-fw"></i>
				<p class="MuiTypography-root MuiTypography-body1 MuiTypography-colorTextPrimary" style="word-break: break-word;">${car}</p>
				<i class="fa-solid fa-closed-captioning fa-w-16 fa-fw"></i>
				<p class="MuiTypography-root MuiTypography-body1 MuiTypography-colorTextPrimary" style="word-break: break-word;">${plate}</p>
        ...
        html += `<div class="notifa-border called dismissed" aria-controls="ping-menu-263" aria-haspopup="true" x="${coordx}" y="${coordy}">
			<div class="notifa-divs">
			   <div class="notifa-header">
				  <div class="MuiChip-root MuiChip-colorPrimary MuiChip-sizeSmall"><span class="MuiChip-label MuiChip-labelSmall">${greenNumber}</span></div>
				  <div class="MuiChip-root MuiChip-colorSecondary MuiChip-sizeSmall" style="margin-left: 8px;"><span class="MuiChip-label MuiChip-labelSmall">${orangeNumber}</span></div>
    ...
    function createNotifyAction2Mod2(coordx, coordy, greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        $('.marked-notifa-main').prepend(createMenuNotifyMod2(coordx, coordy,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass));
    }
    function createNotifyAction2Mod3(coordx, coordy, greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass) {
        $('.marked-notifa-main').prepend(createMenuNotifyMod3(coordx, coordy,greenNumber, orangeNumber, crimeName, address, time, car, plate, carColor, vehicleClass));
    }
    ...
    function createTablePolice(result) {
        ...
        for(var k in result) {
            if (result[k].whounder == false) {
                html += `
                <div class="mesai-notifa-main" saveattr="${result[k].save}">
    ...
    function vehiclesClick() {
        $('.vehicles').click(function() {
            ...
            if (value == 1) {
                ...
                removeItem()
                mesaiHold()
                $.post('https://fa-dispatch/change-vehicle', JSON.stringify({
                    name: policeName,
                    icon: icon
                }));
                ...
                $.post('https://fa-dispatch/table_ayril', JSON.stringify({
                    name: remove
                }));
    ...
    $('.create-call').click(function() {
        createCall2.remove();
        $('.assign-list').fadeOut();
        $('.marked-notifa-main').prepend(createCalls);
        assing();
    })
    ...
    $('.dismiss-ping').click(function() {
        ...
        $.post('https://fa-dispatch/dismiss_call-1', JSON.stringify({
            number: roninNumber
        }));
    })

    function assing() {
        $('.notifa-border').on('mousedown', function(e) {
            ...
            if (check == false) {
                ...
                createCalls = `<div class="notifa-border called" aria-haspopup="true" x="${coords[0]}" y="${coords[1]}">${createCalls}
    ...
    function assignButtons(){
        $('.assing-buttons').click(function() {
            $.post('https://fa-dispatch/add_police-assign', JSON.stringify({
    ...
    $('.crate-dismiss').click(function() {
        ...
        $.post('https://fa-dispatch/create_call-2', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    $('.remove-dismis-ping').click(function() {
        $('.dismiss-list').fadeOut();
        createCall4.remove();
        $.post('https://fa-dispatch/remove-call', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function cureungotunu() {
        ...
        $('.remove-unit').click(function() {
            ...
            $.post('https://fa-dispatch/delete', JSON.stringify({
                name: player
            }));
            ...
            $('.mesai-personel').each(function() {
                if ($.trim($(this).text()) == text) {
                    ...
                    if (owner == player) {
                        ...
                        $.post('https://fa-dispatch/under-police', JSON.stringify({
                            player: player,
                            owner: owner
                        }));
                    }
    ...
    function callCameRemove(){
        $('.call-came').dblclick(function(e){
            ...
            $('.remove-list .MuiPaper-rounded ul li').click(function(){
                $.post('https://fa-dispatch/delete_police-assign', JSON.stringify({
                    ronin: callCame
                }));
            })
        })
    ...
    function save(){
        ...
        $.post('https://fa-dispatch/save', JSON.stringify({
            html: save
        }));
