...
RegisterNUICallback('fa-ui:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'fa-ui:inputResponse') then return end
...
RegisterNUICallback('fa-ui:inputResponse', function(data, cb)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['fa-ui']:openApplication('textbox', {
        callbackUrl = 'fa-ui:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
