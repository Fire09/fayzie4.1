...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('fa-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("fa-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['fa-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("fa-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("fa-fishing:currentlyRentedBoat", function(handle)
